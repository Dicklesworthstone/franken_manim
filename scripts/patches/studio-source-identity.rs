...
impl Project {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!(
            "fmn-source-identity-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)
        ));
    ...
    fn snapshot(&self, extra: &[PathBuf]) -> SourceWatch {
        ...
        SourceWatch::new_filtered(roots, Vec::new(), Duration::from_millis(100), WatchLimits::default(),
            WatchFilter { extensions: vec!["py".into(), "pyw".into()],
                ignored_directories: vec!["__pycache__".into()] }).unwrap()
    }
}
...
fn same_length_helper_edits_change_generation_identity_not_mtime_touches() {
    ...
    fs::File::options().write(true).open(&helper).unwrap().set_modified(modified).unwrap();
...
fn explicit_assets_are_hashed_but_generated_frames_are_not() {
    ...
    let before = project.snapshot(std::slice::from_ref(&asset)).content_fingerprint();
    fs::write(project.0.join("render.png"), "output").unwrap();
    assert_eq!(before, project.snapshot(std::slice::from_ref(&asset)).content_fingerprint());
...
fn files_refer_to_exactly_the_same_frozen_scan_as_the_fingerprint() {
    ...
    assert_eq!(rows[0], (source.as_path(), fmn_studio::protocol_digest(b"one")));
...
fn paths_and_missing_file_kinds_are_part_of_the_identity() {
    ...
    let before = project.snapshot(std::slice::from_ref(&absent)).content_fingerprint();
    fs::write(&absent, "").unwrap();
    let present = project.snapshot(std::slice::from_ref(&absent)).content_fingerprint();
    assert_ne!(before, present);
    fs::write(project.0.join("different.csv"), "").unwrap();
    assert_ne!(present, project.snapshot(&[project.0.join("different.csv")]).content_fingerprint());
}
...
fn root_order_and_duplicate_roots_do_not_change_observed_content_identity() {
    ...
    let second = SourceWatch::new_filtered(vec![source, project.0.clone(), project.0.clone()],
        Vec::new(), Duration::ZERO, WatchLimits::default(), WatchFilter {
