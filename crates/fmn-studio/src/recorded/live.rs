...
impl RecordedTimeline {
    ...
    pub fn replace_live_frame(
        &mut self, stream: FrameStream, inspection: InspectorSnapshot,
    ) -> Result<(), ServiceError> {
        if self.input_revision.is_none() {
            return Err(invalid("no live capture owner"));
        }
        let index = self.frames.len().checked_sub(1).ok_or_else(|| invalid("empty live capture"))?;
    ...
    pub fn prepare_live_input(
        &self, scene: &str, command: &CommandRecord,
    ) -> Result<StudioInput, ServiceError> {
        self.check_scene(scene)?;
        let input = studio_input_payload(scene, command).map_err(failed)?;
        let revision = self.input_revision.ok_or_else(|| invalid("live input is disabled; reload the scene"))?;
        if input.frame != self.frames.len() as i64 - 1 || input.frame != self.position as i64 {
            return Err(invalid("live input requires the selected final frame; historical frames are read-only"));
        }
        if input.revision != revision {
            return Err(invalid("stale live input revision"));
        }
        if input.target.is_some() {
            return Err(invalid("Python input uses scene hit testing, not native object targets"));
        }
    ...
    pub fn commit_live_input(&mut self, command: CommandRecord) -> Result<WorkerResponse, ServiceError> {
        let input = self.prepare_live_input(&self.scene, &command)?;
        let next = input.revision.checked_add(1).ok_or_else(|| invalid("live input revision exhausted"))?;
        let response = self.record_opaque(command)?;
        self.input_revision = Some(next);
        Ok(response)
    }

    pub(super) fn record_opaque(&mut self, command: CommandRecord) -> Result<WorkerResponse, ServiceError> {
        if self.commands >= 4096 {
            return Err(invalid("recorded position journal budget exceeded"));
        }
        ...
        let response = WorkerResponse::JournalSegment {
            scene: self.scene.clone(), start_entry: self.commands,
...
mod tests {
    ...
    fn live_tail_preserves_history_and_rejects_stale_commands_before_execution() {
        let mut timeline = RecordedTimeline::new("s".into(), sha256(b"b"), sha256(b"s"), 2, 1 << 20).unwrap();
        for v in [0, 120] { let (f, i) = super::super::tests::capture("s", v); timeline.push(f, i).unwrap(); }
        timeline.enable_live_input().unwrap();
        let input = StudioInput { frame: 1, revision: 0, target: None,
            event: EventPayload::KeyPress { key: Key::ArrowRight, modifiers: Modifiers::NONE } };
        let command = studio_input_command("s", &input).unwrap();
        assert!(timeline.prepare_live_input("s", &command).is_err());
        timeline.handle(SupervisorRequest::Play { scene: "s".into(), command: studio_seek_command("s", 1).unwrap() }).unwrap();
        timeline.prepare_live_input("s", &command).unwrap();
        let used = timeline.encoded_bytes();
        for _ in 0..10 { let (f, i) = super::super::tests::capture("s", 255); timeline.replace_live_frame(f, i).unwrap(); }
        assert_eq!(timeline.frame_count(), 2);
        assert!(timeline.encoded_bytes() < used + 64);
        let WorkerResponse::JournalSegment { start_entry, journal, .. } = timeline.commit_live_input(command.clone()).unwrap() else { panic!() };
        assert_eq!(start_entry, 1);
        assert!(Journal::from_bytes(&journal).unwrap().entries()[0].is_replay_barrier());
        assert!(timeline.prepare_live_input("s", &command).is_err());
        let WorkerResponse::StudioData { bytes, .. } = timeline.handle(SupervisorRequest::Inspect { scene: "s".into() }).unwrap() else { panic!() };
        assert!(String::from_utf8(bytes).unwrap().contains("\"input_revision\":1"));
        assert!(timeline.prepare_live_input("s", &command).is_err());
        timeline.handle(SupervisorRequest::Scrub { scene: "s".into(), frame: 0 }).unwrap();
        let WorkerResponse::StudioData { bytes, .. } = timeline.handle(SupervisorRequest::Inspect { scene: "s".into() }).unwrap() else { panic!() };
        assert!(String::from_utf8(bytes).unwrap().contains("\"input_events\":false"));
        assert_eq!(timeline.frames[0].state_hash, timeline.last_state_hash().unwrap());
    ...
    fn invalid_live_replacement_retains_complete_previous_capture() {
        let mut timeline = RecordedTimeline::new("s".into(), sha256(b"b"), sha256(b"s"), 1, 1 << 20).unwrap();
        let (f, i) = super::super::tests::capture("s", 120); timeline.push(f, i).unwrap();
        timeline.enable_live_input().unwrap();
        let before = (timeline.last_state_hash(), timeline.encoded_bytes());
        let (mut f, i) = super::super::tests::capture("s", 255);
        f.width += 1;
        assert!(timeline.replace_live_frame(f, i).is_err());
        assert_eq!((timeline.last_state_hash(), timeline.encoded_bytes()), before);
