...
enum Input { Missing, Directory, File(ProtocolDigest) }
...
pub struct SourceWatch {
    inputs: Vec<PathBuf>, excludes: Vec<PathBuf>, limits: WatchLimits,
    debounce: Duration, observed: Snapshot, attempted: Snapshot,
    changed_at: Option<Duration>, last_poll: Option<Duration>,
}
...
impl SourceWatch {
    ...
    pub fn new(
        ...
        Ok(Self { inputs, excludes, limits, debounce, attempted: observed.clone(), observed,
    ...
    pub fn poll(&mut self, now: Duration) -> Result<bool, BuildError> {
        if self.last_poll.is_some_and(|last| now < last) { return Err(error("source-watch clock moved backwards")); }
        let current = scan(&self.inputs, &self.excludes, self.limits)?;
        self.last_poll = Some(now);
        if current != self.observed { self.observed = current; self.changed_at = Some(now); }
        if self.observed == self.attempted { self.changed_at = None; return Ok(false); }
        if self.changed_at.is_some_and(|changed| now.saturating_sub(changed) >= self.debounce) {
            self.attempted = self.observed.clone(); self.changed_at = None; return Ok(true);
        }
        Ok(false)
    }
}
...
fn scan(inputs: &[PathBuf], excludes: &[PathBuf], limits: WatchLimits) -> Result<Snapshot, BuildError> {
    ...
    let mut seen = BTreeSet::new(); let mut snapshot = Vec::new(); let mut remaining = limits.max_bytes;
    while let Some((path, depth)) = work.pop() {
        if excludes.iter().any(|exclude| path.starts_with(exclude)) || seen.contains(&path) { continue; }
        if seen.len() >= limits.max_entries || depth > limits.max_depth { return Err(error("source-watch entry/depth budget exceeded")); }
        seen.insert(path.clone()); snapshot.try_reserve(1).map_err(error)?;
        let metadata = match fs::symlink_metadata(&path) {
            Ok(metadata) => metadata,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => { snapshot.push((path, Input::Missing)); continue; }
            Err(err) => return Err(error(err)),
        };
        if metadata.is_dir() {
            snapshot.push((path.clone(), Input::Directory));
            for child in fs::read_dir(&path).map_err(error)? {
                let child = child.map_err(error)?; let child_path = child.path();
                if matches!(child.file_name().to_str(), Some("target" | ".git")) && child.file_type().map_err(error)?.is_dir() { continue; }
                if work.len().saturating_add(seen.len()) >= limits.max_entries { return Err(error("source-watch pending-entry budget exceeded")); }
                work.try_reserve(1).map_err(error)?; work.push((child_path, depth + 1));
            }
            ...
            let mut bytes = Vec::new(); bytes.try_reserve_exact(usize::try_from(metadata.len()).map_err(error)?).map_err(error)?;
        ...
        } else { return Err(error(format!("source watcher refuses symlink or non-regular input: {}", path.display()))); }
    }
    snapshot.sort_by(|left, right| left.0.cmp(&right.0)); Ok(snapshot)
}
...
mod tests {
    ...
    impl Temp {
        ...
        fn watch(&self) -> SourceWatch { SourceWatch::new(vec![self.0.clone()], Vec::new(), Duration::from_millis(100), WatchLimits::default()).unwrap() }
    }
    ...
    fn stable_content_is_reported_once_and_failed_builds_do_not_spin() {
        ...
        fs::write(&source, "edit while building").unwrap(); assert!(!watch.poll(ms(1200)).unwrap()); assert!(watch.poll(ms(1300)).unwrap());
    }
    #[test]
    ...
    fn directory_creation_deletion_and_output_exclusion() {
        ...
        fs::create_dir(temp.0.join("target")).unwrap(); fs::write(temp.0.join("target/binary"), "not source").unwrap(); assert!(!watch.poll(ms(0)).unwrap());
        ...
        fs::remove_file(temp.0.join("src/new.rs")).unwrap(); assert!(!watch.poll(ms(102)).unwrap()); assert!(watch.poll(ms(202)).unwrap());
    }
    #[test]
