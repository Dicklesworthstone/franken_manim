...
impl RecordedTimeline {
    ...
    pub fn new(
        scene: String, build_id: Digest, source_digest: Digest,
        ...
        if !(1..=100_000).contains(&max_frames) || !(1..=1024 * 1024 * 1024).contains(&max_bytes) {
            return Err(invalid("recorded timeline requires 1..100000 frames and at most 1 GiB"));
        }
        Ok(Self {
            scene, build_id, source_digest, frames: Vec::new(), position: 0,
            encoded_bytes: 0, max_frames, max_bytes, commands: 0, limits,
        })
    }
    ...
    pub fn push(
        &mut self, mut stream: FrameStream, mut inspection: InspectorSnapshot,
    ) -> Result<(), ServiceError> {
        ...
        if stream.scene != self.scene || stream.encoding != FrameEncoding::Png {
            return Err(invalid("recorded timeline requires same-scene PNG captures"));
        }
        stream.validate(self.limits).map_err(failed)?;
        let FramePayload::Pipe { bytes, .. } = &stream.payload else {
            return Err(invalid("recorded timelines cannot retain shared-memory tokens"));
        };
        if bytes.len() > self.max_bytes.saturating_sub(self.encoded_bytes) {
            return Err(invalid("recorded timeline byte budget exceeded"));
        }
        let png = decode_png(bytes, &PngLimits {
        ...
        let view = inspection.view.as_mut().ok_or_else(|| invalid("capture has no native inspector view"))?;
        if view.width != stream.width || view.height != stream.height || view.input_events {
            return Err(invalid("recorded inspector geometry or input policy disagrees with capture"));
        }
        if let Some(first) = self.frames.first() {
            let previous = first.inspection.view.as_ref().expect("admitted capture has a view");
            if (view.width, view.height, view.fps) != (previous.width, previous.height, previous.fps) {
        ...
        let document = inspection.to_json(InspectorLimits::default()).map_err(failed)?;
        ...
        let cost = bytes.len().checked_add(document.len()).and_then(|n| n.checked_add(4096))
        ...
        identity.put_bytes(self.source_digest.as_bytes()).put_u64(index)
            .put_bytes(sha256(bytes).as_bytes()).put_bytes(sha256(&document).as_bytes());
        let state_hash = sha256(&identity.finish().map_err(failed)?);
        self.frames.try_reserve(1).map_err(failed)?;
        self.frames.push(RecordedFrame { stream, inspection, state_hash });
        self.encoded_bytes = cost;
        Ok(())
    }
    ...
    #[must_use]
    pub const fn encoded_bytes(&self) -> usize { self.encoded_bytes }
    ...
    fn select(&mut self, scene: &str, frame: i64) -> Result<(), ServiceError> {
        ...
        if index >= self.frames.len() { return Err(invalid("frame is outside the captured timeline")); }
        self.position = index;
        Ok(())
    }
    ...
    fn check_scene(&self, scene: &str) -> Result<(), ServiceError> {
        if scene != self.scene {
            return Err(ServiceError::new(WorkerErrorCode::SceneNotFound, "unknown captured scene"));
        }
        if self.frames.is_empty() { return Err(invalid("recorded timeline is empty")); }
        Ok(())
    }

    fn frame(&self) -> Result<WorkerResponse, ServiceError> {
        let source = &self.frames[self.position].stream;
        let FramePayload::Pipe { bytes, digest } = &source.payload else { unreachable!() };
        let mut copy = Vec::new();
        copy.try_reserve_exact(bytes.len()).map_err(failed)?;
        copy.extend_from_slice(bytes);
        let render_backends = source.render_backends.iter().map(|b| b.try_clone().map_err(failed))
            .collect::<Result<_, _>>()?;
        Ok(WorkerResponse::Frame(FrameStream {
            scene: self.scene.clone(), frame_index: self.position as u64,
            width: source.width, height: source.height, stride: 0, encoding: FrameEncoding::Png,
            payload: FramePayload::Pipe { bytes: copy, digest: *digest }, render_backends,
        }))
    }
}
...
impl WorkerService for RecordedTimeline {
    ...
    fn last_state_hash(&self) -> Option<Digest> { self.frames.get(self.position).map(|f| f.state_hash) }
    ...
    fn begin_session(&mut self, _: Digest, max_frame_bytes: usize) -> Result<(), ServiceError> {
        ...
        for frame in &self.frames { frame.stream.validate(self.limits).map_err(failed)?; }
        Ok(())
    }
    ...
    fn handle(&mut self, request: SupervisorRequest) -> Result<WorkerResponse, ServiceError> {
        match request {
            ...
            SupervisorRequest::Scrub { scene, frame } | SupervisorRequest::Seek { scene, frame } => {
            ...
            SupervisorRequest::Inspect { scene } => {
                ...
                snapshot.view.as_mut().expect("admitted capture has a view").frame_count = count;
                let bytes = snapshot.to_json(InspectorLimits {
                    max_json_bytes: self.limits.max_studio_data_bytes,
                    ..InspectorLimits::default()
                }).map_err(failed)?;
                let digest = sha256(&bytes);
                Ok(WorkerResponse::StudioData { scene, kind: StudioDataKind::Inspection, bytes, digest })
            }
            SupervisorRequest::Play { scene, command } => {
                if self.commands >= 4096 { return Err(invalid("recorded position journal budget exceeded")); }
                ...
                let result = (|| {
                    journal.record(Entry {
                        command, effect: EffectClass::Opaque,
                        reads: vec![AssetRead { path: "portal/capture-generation".into(), digest: self.source_digest }],
                        subprocesses: Vec::new(), checkpoint: None,
                        state_hash: self.frames[self.position].state_hash,
                    }).map_err(failed)?;
                    let journal = journal.to_bytes().map_err(failed)?;
                    Ok(WorkerResponse::JournalSegment { scene, start_entry: self.commands, journal })
                })();
                if result.is_ok() { self.commands += 1; } else { self.position = previous; }
                result
            }
            SupervisorRequest::RestoreCheckpoint(_) => Err(ServiceError::new(
                WorkerErrorCode::CheckpointRejected, "captured previews do not restore authored callbacks; reload the source")),
            SupervisorRequest::ReplayJournal(_) => Err(ServiceError::new(
                WorkerErrorCode::ReplayFailed, "captured Python execution is opaque; source reload must execute a fresh worker")),
            SupervisorRequest::Event { .. } => Err(invalid("captured preview is read-only; live Python event editing is not connected")),
            SupervisorRequest::Overlay { .. } => Err(invalid("captured camera overlays are not available")),
            SupervisorRequest::Hello { .. } | SupervisorRequest::Shutdown => Err(invalid("request belongs to the worker protocol driver")),
        }
    }
}
...
pub fn start_host(
    scene: &str, mut builder: Box<dyn RebuildDriver + Send>, token: CapabilityToken,
    config: StudioHostConfig, request_timeout: Duration,
    ...
    let store = Store::open(Arc::new(VirtualFs::new()), Arc::clone(&clock),
        std::env::temp_dir().join("fmn-recorded-studio-cache"), StoreConfig::default())
        .map_err(|e| HostError::Io(std::io::Error::other(e.to_string())))?;
    let cache = store.namespace("captured-preview", 1, NamespacePolicy { ceiling_bytes: Some(8 * 1024 * 1024) })
        .map_err(|e| HostError::Io(std::io::Error::other(e.to_string())))?;
    let mut supervisor = Supervisor::new(Box::new(StdWorkerLauncher::default()), Arc::clone(&clock), cache,
        SupervisorConfig { request_timeout, auto_restart: false, ..SupervisorConfig::default() });
    supervisor.install_session(scene, Journal::new())?;
    supervisor.build_and_start(&mut *builder)?;
    let reply = supervisor.request(SupervisorRequest::Scrub { scene: scene.into(), frame: 0 }, &|_| false)?;
    ...
    let session = Arc::new(StudioWorkerSession::new(scene, supervisor, builder, Arc::new(|_| false))?);
...
mod tests {
    ...
    fn capture(scene: &str, value: u8) -> (FrameStream, InspectorSnapshot) {
        let stage = fmn_scene::studio_bridge::Stage::new();
        let viewport = Viewport { width: 2, height: 2 };
        let map = ScreenMap { scale: 1.0, origin: [1.0, 1.0] };
        let config = FrameConfig::new(viewport, map, fmn_core::color::LinearRgba { r: 0.0, g: 0.0, b: 0.0, a: 1.0 });
        let backend = RenderBackendRecord::new(RenderBackendRole::FrameStream,
            fmn_render::engine::journal(EngineIdentity::certified(), &config, Tiling { macro_tile: 64, fine_tile: 8 })).unwrap();
        let bytes = fmn_codec::encode_rgba8(2, 2, &[value; 16], fmn_codec::CompressionLevel::Fast);
        let digest = sha256(&bytes);
        let stream = FrameStream { scene: scene.into(), frame_index: 100,
            width: 2, height: 2, stride: 0, encoding: FrameEncoding::Png,
            payload: FramePayload::Pipe { bytes, digest }, render_backends: vec![backend] };
        let mut inspect = InspectorSnapshot::capture(&stage, &SpanRegistry::new(), InspectorLimits::default()).unwrap();
    ...
    fn recorded_timeline_scrubs_real_pngs_and_matching_native_inspection() {
        let mut worker = RecordedTimeline::new("scene".into(), sha256(b"build"), sha256(b"source"), 3, 1 << 20).unwrap();
        for value in [0, 120, 255] { let (f, i) = capture("scene", value); worker.push(f, i).unwrap(); }
        worker.begin_session(sha256(b"host"), 1024).unwrap();
        for index in [2, 0, 1, 2] {
            let WorkerResponse::Frame(frame) = worker.handle(SupervisorRequest::Scrub { scene: "scene".into(), frame: index }).unwrap() else { panic!() };
            assert_eq!(frame.frame_index, index as u64);
            let FramePayload::Pipe { bytes, .. } = frame.payload else { panic!() };
            assert_eq!(decode_png(&bytes, &PngLimits::default()).unwrap().rgba[0], [0, 120, 255][index as usize]);
            let WorkerResponse::StudioData { bytes, .. } = worker.handle(SupervisorRequest::Inspect { scene: "scene".into() }).unwrap() else { panic!() };
        ...
        let WorkerResponse::JournalSegment { journal, .. } = worker.handle(SupervisorRequest::Play { scene: "scene".into(), command }).unwrap() else { panic!() };
    ...
    fn recorded_timeline_refusals_are_atomic_and_bounded() {
        let mut worker = RecordedTimeline::new("scene".into(), sha256(b"b"), sha256(b"s"), 1, 1 << 20).unwrap();
        let (frame, inspection) = capture("other", 0);
        assert!(worker.push(frame, inspection).is_err());
        assert_eq!(worker.frame_count(), 0);
        let (frame, inspection) = capture("scene", 0); worker.push(frame, inspection).unwrap();
        let used = worker.encoded_bytes();
        let (frame, inspection) = capture("scene", 1); assert!(worker.push(frame, inspection).is_err());
        assert_eq!(worker.encoded_bytes(), used);
        assert!(worker.handle(SupervisorRequest::Scrub { scene: "scene".into(), frame: 1 }).is_err());
        assert!(worker.handle(SupervisorRequest::Scrub { scene: "other".into(), frame: 0 }).is_err());
        assert!(worker.begin_session(sha256(b"h"), 1).is_err());
        let mut tiny = RecordedTimeline::new("scene".into(), sha256(b"b"), sha256(b"s"), 1, 1).unwrap();
        let (frame, inspection) = capture("scene", 0); assert!(tiny.push(frame, inspection).is_err());
