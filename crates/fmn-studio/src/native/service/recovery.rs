...
impl NativeSceneWorker {
    pub(super) fn replay(&mut self, replay: JournalReplay) -> Result<WorkerResponse, ServiceError> {
        ...
        for (offset, entry) in entries.iter().enumerate() {
            ...
            let frame = self.prepare_command(&entry.command, position, &mut simulated)
            ...
            total_inputs = total_inputs.checked_add(simulated.visible_count(frame)).ok_or_else(|| refuse("native input replay work overflows"))?;
            if total_frames > self.config.max_replay_frames || total_inputs > self.config.max_replay_inputs {
                return Err(refuse("native replay exceeds its total frame or input work budget"));
            }
        ...
        for (offset, entry) in entries.iter().enumerate() {
            ...
            let mut program = self.fresh_at(frame, &edits).map_err(|error| refuse_owned(WorkerErrorCode::ReplayFailed, error))?;
    ...
    pub(super) fn restore(&mut self, checkpoint: Checkpoint) -> Result<WorkerResponse, ServiceError> {
        ...
        let restored = decode_state(&checkpoint.state, &self.config.scene, next, &self.reads,
            self.config.max_recorded_inputs, self.config.frame_count, self.config.limits.max_checkpoint_bytes)
            .map_err(|error| refuse_owned(WorkerErrorCode::CheckpointRejected, error))?;
        if !restored.edits.is_empty() && !self.input_enabled() {
            return Err(refuse("edited checkpoint requires native committed input support"));
        }
        ...
        if decoded.fps != self.config.fps { return Err(refuse("native checkpoint uses a different frame clock")); }
        ...
        if state != checkpoint.state {
            return Err(refuse("native checkpoint does not match fresh callback and input execution"));
        }
