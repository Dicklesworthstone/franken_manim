...
use fmn_render::{Binning, Camera, CameraConfig, RetainedFrameRenderer, RetainedFrameRendererConfig, ScreenMap};
...
impl NativeRaster {
    pub(super) fn new(config: RetainedFrameRendererConfig, base_camera: Option<CameraConfig>) -> Result<Self, ServiceError> {
        let camera = base_camera.clone().map(|camera| CameraCapture::new(camera, config)).transpose()?;
        ...
        Ok(Self {
            ...
            rgba: FrameBuffer::new(layout), camera, base_camera, rig_backend: None,
        })
    }
    ...
    pub(super) fn bind_camera(&mut self, index: Option<u64>) -> Result<(), ServiceError> {
        self.rig_backend = if let Some(index) = index {
            let camera = self.camera.as_ref().ok_or_else(|| invalid("camera rig requires an explicit worker CameraConfig"))?;
            let mut writer = Writer::new(Schema::new(*b"FMRC", 1, 1, 0));
            writer.put_str("lumen-native-camera-rig-v1").put_u64(index).put_bytes(camera.backend.identity());
            Some(RenderBackendRecord::new(RenderBackendRole::FrameStream, writer.finish().map_err(execution_error)?)
                .map_err(execution_error)?)
        } else { None };
        Ok(())
    }
    ...
    fn sampled_camera(&self, program: &NativeSceneProgram) -> Result<Option<Camera>, ServiceError> {
        match program.camera_rig() {
            Some(rig) => {
                let base = self.base_camera.as_ref().ok_or_else(|| invalid("camera rig requires an explicit worker CameraConfig"))?;
                let config = rig.sample(program.preview().stage(), base).map_err(execution_error)?;
    ...
    pub(super) fn frame(&mut self, scene: &str, program: &NativeSceneProgram) -> Result<WorkerResponse, ServiceError> {
        if let Some(sample) = self.sampled_camera(program)? {
            let capture = self.camera.as_mut().ok_or_else(|| invalid("missing native camera capture"))?;
            ...
            if capture.camera.light_source_position() != sample.light_source_position() {
                capture.camera.set_light_source_position(sample.light_source_position()).map_err(execution_error)?;
            }
            self.renderer.render_with_camera(program.preview().stage(), &capture.camera).map_err(execution_error)?;
        } else {
    ...
    pub(super) fn backend(&self) -> Result<RenderBackendRecord, ServiceError> {
        if let Some(backend) = &self.rig_backend { return backend.try_clone().map_err(execution_error); }
        if let Some(camera) = &self.camera { return camera.backend.try_clone().map_err(execution_error); }
        let config = self.renderer.config();
    ...
    pub(super) fn inspect(&self, program: &NativeSceneProgram, frame_count: u64, max_bytes: usize) -> Result<Vec<u8>, ServiceError> {
        ...
        let mut snapshot = InspectorSnapshot::capture(program.preview().stage(), program.spans(), limits).map_err(execution_error)?;
        let config = self.renderer.config();
        let map = self.sampled_camera(program)?.map_or(config.frame.map, |camera| ScreenMap {
            scale: 1.0 / camera.pixel_size(),
            origin: [f64::from(camera.pixel_width()) * 0.5, f64::from(camera.pixel_height()) * 0.5],
        });
        snapshot.view = Some(InspectorView::new(program.frame_index(), frame_count,
            program.preview().scene().fps(), config.frame.viewport, map, self.camera.is_none()).map_err(execution_error)?);
    ...
    pub(super) fn overlay(&mut self, program: &NativeSceneProgram, layers: DebugLayerSet, max_bytes: usize) -> Result<Vec<u8>, ServiceError> {
        if self.camera.is_some() {
            return Err(invalid("native camera overlays require projected diagnostics; affine overlays are unavailable"));
        }
        self.renderer.render(program.preview().stage(), 0).map_err(execution_error)?;
        let config = self.renderer.config();
        let mut binning = Binning::build(self.renderer.plan(), config.frame.viewport, config.tiling, config.frame.map).map_err(execution_error)?;
