...
impl EditTrack {
    ...
    pub fn try_clone(&self) -> Result<Self, ServiceError> {
        ...
        rows.try_reserve_exact(self.rows.len()).map_err(execution_error)?;
        for row in &self.rows {
            let mut label = String::new();
            label.try_reserve_exact(row.command.label.len()).map_err(execution_error)?;
            label.push_str(&row.command.label);
            rows.push(InputRow {
                command: CommandRecord { kind: row.command.kind, identity: row.command.identity, label },
    ...
    pub fn append(
        ...
        if input.revision != position || input.target.is_some() {
            return Err(invalid("stale native input revision or unsupported explicit target"));
        }
        let keep = self.rows.partition_point(|row| row.input.frame <= input.frame);
        if keep >= limit {
            return Err(invalid("native committed input track exceeds its event budget"));
        }
        let mut label = String::new();
        label.try_reserve_exact(command.label.len()).map_err(execution_error)?;
        ...
        self.rows.push(InputRow {
            command: CommandRecord { kind: command.kind, identity: command.identity, label }, input,
        });
        Ok(frame)
    }

    pub fn visible_count(&self, target: u64) -> u64 {
        self.rows.partition_point(|row| row.input.frame.cast_unsigned() <= target) as u64
    }
    ...
    pub fn advance(
        ...
        for row in &self.rows {
            ...
            if after.is_some_and(|previous| frame <= previous) { continue; }
...
pub(super) fn encode_state(
    ...
    if edits.is_empty() { return Ok(state); }
    let mut writer = Writer::with_limits(EDIT_STATE_SCHEMA, Limits { max_total: max_bytes, max_field: max_bytes });
    writer.put_str(scene).put_u64(position).put_u32(reads.len() as u32);
    for read in reads { writer.put_str(&read.path).put_bytes(read.digest.as_bytes()); }
    writer.put_u32(edits.rows.len() as u32);
    for row in &edits.rows {
        writer.put_bytes(row.command.identity.as_bytes()).put_str(&row.command.label);
    }
...
pub(super) fn decode_state<'a>(
    ...
    if bytes.len() > max_bytes { return Err(invalid("native checkpoint exceeds its byte budget")); }
    if !bytes.starts_with(b"FMEI") {
        // The caller still validates this through SceneState's strict reader.
        return Ok(RestoredTrack { edits: EditTrack::default(), state: bytes });
    }
    let mut reader = Reader::open(bytes, EDIT_STATE_SCHEMA,
        Limits { max_total: max_bytes, max_field: max_bytes }, UnknownPolicy::Strict).map_err(execution_error)?;
    if reader.get_str().map_err(execution_error)? != scene || reader.get_u64().map_err(execution_error)? != position {
        return Err(invalid("edited checkpoint scene or journal position differs"));
    }
    ...
    edits.rows.try_reserve_exact(count).map_err(execution_error)?;
    for _ in 0..count {
        let digest: [u8; 32] = reader.get_bytes().map_err(execution_error)?.try_into().map_err(execution_error)?;
        ...
        owned.try_reserve_exact(label.len()).map_err(execution_error)?;
        owned.push_str(label);
        let command = CommandRecord { kind: CommandKind::Custom, identity: Digest::from_bytes(digest), label: owned };
        let input = studio_input_payload(scene, &command).map_err(execution_error)?;
        if input.target.is_some() || input.revision >= position || input.frame.cast_unsigned() >= frame_count
            || edits.rows.last().is_some_and(|row| row.input.frame > input.frame || row.input.revision >= input.revision)
        { return Err(invalid("edited checkpoint input ordering or target is invalid")); }
