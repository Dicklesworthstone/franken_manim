    ...
    for entry in journal.entries() {
        if let Some(bytes) = &entry.checkpoint {
            if bytes.len() > checkpoint_limit {
                return Err(SupervisorError::InvalidSession("saved checkpoint exceeds protocol budget"));
            }
            // Public content-integrity identifiers, not authentication secrets.
            if sha256(bytes) != entry.state_hash {
                return Err(SupervisorError::InvalidSession("saved checkpoint checksum mismatch"));
            }
        }
    }
    Ok(())
...
impl SavedSession {
    ...
    pub fn from_bytes(bytes: &[u8], limits: ProtocolLimits) -> Result<Self, SupervisorError> {
        let mut reader = Reader::open(bytes, SAVED_SESSION_SCHEMA, container_limits(limits), UnknownPolicy::Strict)?;
        ...
        if journal_bytes.len() > journal_limit(limits) {
            return Err(SupervisorError::InvalidSession("saved journal exceeds protocol budget"));
        }
        ...
        if journal.entries().is_empty() && committed_frame != 0 {
            return Err(SupervisorError::InvalidSession("nonzero saved frame without a committed journal"));
        }
        Ok(Self { scene, worker_build, context, committed_frame, journal })
    }
    ...
    pub fn to_bytes(&self, limits: ProtocolLimits) -> Result<Vec<u8>, SupervisorError> {
        ...
        if self.committed_frame < 0 || (self.journal.entries().is_empty() && self.committed_frame != 0) {
            return Err(SupervisorError::InvalidSession("invalid saved committed frame"));
        }
        ...
        writer.put_str(&self.scene).put_digest(&self.worker_build).put_digest(&self.context)
            .put_i64(self.committed_frame).put_bytes(&bytes);
        Ok(writer.finish()?)
    }

    /// The last committed position, not a transient scrub/playback preview.
    #[must_use]
    pub const fn committed_frame(&self) -> i64 { self.committed_frame }
    ...
    pub fn validate_context(&self, scene: &str, context: Digest) -> Result<(), SupervisorError> {
        if self.scene != scene || self.context != context {
            return Err(SupervisorError::InvalidSession("saved scene or render configuration does not match"));
        }
        Ok(())
    }
}
...
impl Supervisor {
    ...
    pub fn save_session(
        ...
        let worker_build = self.artifact.as_ref().ok_or(SupervisorError::NoWorker)?.build_id;
        require_replayable(&self.journal, asset_ok)?;
        SavedSession {
            scene: try_clone_string(scene, "exported session scene")?,
            worker_build, context, committed_frame, journal: self.journal.try_clone()?,
    ...
    pub fn resume_saved_session(
        ...
        if self.generation != 0 || self.scene.is_some() || self.worker.is_some() {
            return Err(SupervisorError::InvalidSession("resume requires a fresh supervisor"));
        }
        ...
        if artifact.build_id != saved.worker_build {
            return Err(SupervisorError::InvalidSession("saved worker build does not match the selected executable"));
        }
        ...
        match self.recover(&commands, asset_ok, started) {
            Ok(report) if report.plan.reuse == commands.len() && report.diverged_at.is_none() && !report.cold_fallback => Ok(report),
            Ok(_) => {
                self.shutdown_worker();
                Err(SupervisorError::InvalidSession("saved session replay diverged; refusing partial recovery"))
            }
            Err(error) => { self.shutdown_worker(); Err(error) }
        }
    }
}
...
mod tests {
    ...
    fn saved() -> SavedSession {
        ...
        journal.record(Entry {
            command: CommandRecord { kind: CommandKind::Play, identity: sha256(b"command"), label: "a committed command".into() },
            effect: EffectClass::Pure,
            reads: vec![AssetRead { path: "native/source".into(), digest: sha256(b"source") }],
            subprocesses: vec![], checkpoint: Some(b"checkpoint".to_vec()), state_hash: sha256(b"checkpoint"),
        }).unwrap();
        SavedSession { scene: "Scene Δ".into(), worker_build: sha256(b"build"), context: sha256(b"config"), committed_frame: 12, journal }
    }
    ...
    fn saved_session_roundtrip_is_canonical_and_self_contained() {
        ...
        assert_eq!(decoded.journal.entries()[0].checkpoint.as_deref(), Some(b"checkpoint".as_slice()));
        decoded.validate_context("Scene Δ", sha256(b"config")).unwrap();
    }
    ...
    fn saved_session_corruption_truncation_and_unknown_fields_refuse() {
        let limits = ProtocolLimits::default();
        let bytes = saved().to_bytes(limits).unwrap();
        for end in [0, 4, bytes.len() - 1] { assert!(SavedSession::from_bytes(&bytes[..end], limits).is_err()); }
        let mut damaged = bytes.clone(); damaged[40] ^= 1;
        assert!(SavedSession::from_bytes(&damaged, limits).is_err());
        let mut writer = Writer::new(SAVED_SESSION_SCHEMA);
        let saved = saved();
        writer.put_str(&saved.scene).put_digest(&saved.worker_build).put_digest(&saved.context)
            .put_i64(12).put_bytes(&saved.journal.to_bytes().unwrap()).put_u8(99);
    ...
    fn saved_session_uses_receivers_journal_and_checkpoint_budgets() {
        ...
        for limits in [ProtocolLimits { max_journal_bytes: 1, ..ProtocolLimits::default() },
            ProtocolLimits { max_checkpoint_bytes: 1, ..ProtocolLimits::default() },
            ProtocolLimits { max_field_bytes: 1, ..ProtocolLimits::default() }] {
    ...
    fn saved_session_cannot_change_scene_config_or_ignore_changed_assets() {
        ...
        assert!(saved.validate_context(&saved.scene, sha256(b"changed seed")).is_err());
        assert!(require_replayable(&saved.journal, &|_| false).is_err());
        assert_eq!(require_replayable(&saved.journal, &|read| read.digest == sha256(b"source")).unwrap().len(), 1);
    }
    ...
    fn saved_session_rejects_bad_checkpoint_even_with_valid_outer_checksum() {
        ...
        entry.state_hash = sha256(b"forged"); journal.record(entry).unwrap();
        let mut writer = Writer::new(SAVED_SESSION_SCHEMA);
        writer.put_str(&saved.scene).put_digest(&saved.worker_build).put_digest(&saved.context)
            .put_i64(12).put_bytes(&journal.to_bytes().unwrap());
        assert!(SavedSession::from_bytes(&writer.finish().unwrap(), limits).is_err());
    }
    ...
    fn saved_session_rejects_opaque_effects_instead_of_claiming_recovery() {
        let mut journal = Journal::new();
        let mut entry = saved().journal.entries()[0].clone();
        entry.effect = EffectClass::Opaque; journal.record(entry).unwrap();
    ...
    fn saved_session_empty_zero_is_valid_but_negative_or_uncommitted_frames_refuse() {
        ...
        assert!(saved.to_bytes(limits).is_err());
        saved.committed_frame = 12; saved.journal = Journal::new();
        assert!(saved.to_bytes(limits).is_err());
        saved.committed_frame = 0;
        let bytes = saved.to_bytes(limits).unwrap();
        assert_eq!(SavedSession::from_bytes(&bytes, limits).unwrap().committed_frame(), 0);
    }
}
