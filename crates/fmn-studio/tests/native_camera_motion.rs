...
use fmn_render::{Camera, CameraConfig, CameraFrame, EngineIdentity, FrameConfig, RetainedFrameRenderer,
    RetainedFrameRendererConfig, ScreenMap, Tiling, Viewport};
use fmn_scene::studio_bridge::FramePacket;
use fmn_scene::{CameraRig, CaptureReason, IntegrationError, Journal, PlayOverrides, RuntimeConfig, Scene, SceneSink};
...
use fmn_studio::{Checkpoint, FramePayload, FrameStream, JournalReplay, ServiceError, SupervisorRequest,
...
fn policy(fps: u32, threads: usize) -> NativeWorkerConfig {
    ...
    let renderer = RetainedFrameRendererConfig {
        frame: FrameConfig::new(Viewport { width: 64, height: 64 },
            ScreenMap { scale: 16.0, origin: [32.0, 32.0] }, background),
        tiling: Tiling { macro_tile: 32, fine_tile: 8 }, engine: EngineIdentity::certified(), threads,
    };
    let mut frame = CameraFrame::default();
    frame.set_shape([4.0, 4.0]).unwrap();
    let mut config = NativeWorkerConfig::new(NAME, protocol_digest(b"rig-build"),
        protocol_digest(b"rig-program-v1"), u64::from(fps) + 1, fps, renderer);
    config.camera = Some(CameraConfig { resolution: (64, 64), fps, background, frame, ..CameraConfig::default() });
...
fn source(fps: u32, fixed: bool) -> (Scene, CameraRig, Mob, Vec<Box<dyn Animation>>) {
    ...
    let mut scene = Scene::new(RuntimeConfig { fps, ..RuntimeConfig::default() }, 37).unwrap();
    ...
    let body = scene.add_mobject(Mobject::from_buffer(buffer).with_uniforms(Uniforms {
        is_fixed_in_frame: if fixed { 1.0 } else { 0.0 }, ..Uniforms::default()
    })).unwrap();
    let rig = CameraRig::new(&mut scene, &base).unwrap();
    // A mutable closure continues after the camera Transform resumes its root.
    let mut tick = 0_u32;
    scene.stage_mut().add_dt_updater(rig.center()[0], move |stage, handle, dt| {
        tick += 1;
        let x = stage.tracker_value(handle).unwrap();
        stage.set_tracker_value(handle, x + f64::from(tick) * dt * 0.1).unwrap();
    }, false).unwrap();
    ...
    pose.set_euler_angles(Some(0.3), Some(0.4), Some(0.2)).unwrap();
    let q = pose.orientation();
    let values = [0.8, 0.2, 0.1, 2.0, q[0], q[1], q[2], q[3], 0.9, 5.0, 2.0, 8.0];
    for (handle, value) in handles.into_iter().zip(values) {
        scene.stage_mut().set_tracker_value(handle, value).unwrap();
    }
    (scene, rig, body, vec![Box::new(Transform::new(rig.root(), target))])
}
...
fn program(fps: u32, fixed: bool) -> Result<NativeSceneProgram, ServiceError> {
    ...
    NativeSceneProgram::new(scene, vec![
        NativeSegment::Play { animations, overrides: PlayOverrides { run_time: Some(0.5), ..PlayOverrides::default() } },
        NativeSegment::Wait { duration: Some(0.5) },
    ], u64::from(fps))?.with_camera_rig(rig)
}
...
fn frame(worker: &mut NativeSceneWorker, index: i64) -> FrameStream {
    let response = worker.handle(SupervisorRequest::Scrub { scene: NAME.into(), frame: index }).unwrap();
    let WorkerResponse::Frame(frame) = response else { panic!("camera frame") };
    frame
}

fn png(frame: &FrameStream) -> &[u8] {
    let FramePayload::Pipe { bytes, .. } = &frame.payload else { panic!("PNG pipe") };
    bytes
}
...
fn every_camera_animation_capture_matches_ordinary_scene_playback_at_two_rates() {
    for fps in [8, 30] {
        ...
        scene.play(animations, PlayOverrides { run_time: Some(0.5), ..PlayOverrides::default() }, &mut captures).unwrap();
        ...
        for packet in captures.0 {
            ...
            assert_eq!(stage.get_bounding_box(body), body_bounds, "only the camera is animated");
            let expected = direct(&stage, rig, &policy);
            let actual = frame(&mut worker, packet.frame_index());
            assert_eq!(png(&actual), expected, "frame {} at {fps}fps", packet.frame_index());
            assert_eq!(actual.render_backends, initial.render_backends, "pose is state, not an unbounded backend list");
        }
...
fn fixed_frame_geometry_stays_fixed_during_camera_translation_zoom_and_rotation() {
    ...
    let initial = frame(&mut worker, 0);
    for index in 1..=8 { assert_eq!(png(&frame(&mut worker, index)), png(&initial)); }
}
...
fn camera_sampling_does_not_modify_native_state_or_repeat_updater_ticks() {
    ...
    let stage = worker.program().unwrap().preview().stage().snapshot().to_bytes().unwrap();
    for _ in 0..3 {
        assert_eq!(frame(&mut worker, 6), before);
        assert_eq!(worker.program().unwrap().preview().stage().snapshot().to_bytes().unwrap(), stage);
    }
    assert_eq!(frame(&mut worker, 2), frame(&mut self::worker(8, 1, false), 2));
}
...
fn moving_camera_frames_and_identities_match_across_render_teams() {
    ...
    for threads in [4, 16] {
        ...
        for (index, expected) in expected.iter().enumerate() {
            assert_eq!(&frame(&mut parallel, i64::try_from(index).unwrap()), expected);
        }
    }
}
...
fn commit(worker: &mut NativeSceneWorker, index: i64) -> Journal {
    let WorkerResponse::JournalSegment { journal, .. } = worker.handle(SupervisorRequest::Play {
        scene: NAME.into(), command: studio_seek_command(NAME, index).unwrap(),
...
fn checkpoint_recovery_and_replay_continue_the_reconstructed_camera_closure() {
    ...
    assert!(entry.reads.iter().any(|read| read.path == "native/camera-rig-binding"));
    ...
    restored.handle(SupervisorRequest::RestoreCheckpoint(Checkpoint {
        scene: NAME.into(), after_entry: 0, state_hash: entry.state_hash, state: entry.checkpoint.clone().unwrap(),
    })).unwrap();
...
fn a_rig_requires_camera_policy_and_cannot_be_bound_late_or_to_another_scene() {
    let mut config = policy(8, 1); config.camera = None;
    assert!(NativeSceneWorker::new(config, || program(8, false)).is_err());
    let (scene, rig, _, animations) = source(8, false);
    let mut program = NativeSceneProgram::new(scene, vec![NativeSegment::Play {
        animations, overrides: PlayOverrides::default(),
    }], 8).unwrap();
    program.advance_to(1).unwrap();
    assert!(program.with_camera_rig(rig).is_err());
    assert!(NativeSceneProgram::new(Scene::default(), Vec::new(), 1).unwrap().with_camera_rig(rig).is_err());
}

#[test]
fn invalid_camera_updates_cannot_publish_frames_or_successful_checkpoints() {
    let factory = || {
        let mut scene = Scene::new(RuntimeConfig { fps: 8, ..RuntimeConfig::default() }, 17).unwrap();
        let rig = CameraRig::new(&mut scene, &policy(8, 1).camera.unwrap()).unwrap();
        scene.stage_mut().add_updater(rig.width(), |stage, width| {
            stage.set_tracker_value(width, 0.0).unwrap();
        }, false).unwrap();
        NativeSceneProgram::new(scene, vec![NativeSegment::Wait { duration: Some(1.0) }], 8)?.with_camera_rig(rig)
    };
    ...
    assert!(worker.handle(SupervisorRequest::Play { scene: NAME.into(), command: studio_seek_command(NAME, 1).unwrap() }).is_err());
...
fn inconsistent_factory_rig_selection_is_rejected_before_execution() {
    ...
    let mut worker = NativeSceneWorker::new(policy(8, 1), move || {
        let iteration = observed.get(); observed.set(iteration + 1);
    ...
    let error = worker.handle(SupervisorRequest::Play { scene: NAME.into(), command: studio_seek_command(NAME, 1).unwrap() }).unwrap_err();
