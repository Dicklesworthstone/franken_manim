...
fn scene(seed: u64) -> Scene {
    Scene::new(RuntimeConfig { fps: 8, ..RuntimeConfig::default() }, seed).unwrap()
}
...
fn deferred_random_choices_consume_the_exact_ordinary_scene_substream() {
    for seed in [0, 1, 71] {
        ...
        ordinary.stage_mut().set_tracker_value(value, expected_value).unwrap();
        ordinary.wait(Some(0.5), &mut NullSceneSink).unwrap();
        ...
        let mut program = NativeSceneProgram::new(source, vec![
            ...
            NativeSegment::edit(move |context| {
                ...
                context.stage_mut().set_tracker_value(value, first + f64::from(second.to_le_bytes()[0]))?;
                Ok(())
            }),
            NativeSegment::Wait { duration: Some(0.5) },
        ], 9).unwrap();
        ...
        assert_eq!(program.preview().stage().tracker_value(value), Some(expected_value));
        program.state_bytes().unwrap();
        program.advance_to(8).unwrap();
...
fn branching(seed: u64) -> NativeSceneProgram {
    NativeSceneProgram::new(scene(seed), vec![NativeSegment::defer(|context| {
        let value = context.rng_mut().next_f64();
        let count = if value < 0.5 { 2 } else { 3 };
        for index in 0..count {
            let mob = context.stage_mut().add(Mobject::from_points(&[[f64::from(index), value, 0.0]]));
            context.stage_mut().add_to_scene(mob)?;
        }
        Ok(vec![NativeSegment::Wait { duration: Some(0.5) }])
    })], 4).unwrap()
}
...
fn seeded_control_flow_reconstructs_the_same_native_topology_and_rng_state() {
    ...
    assert_ne!(first.state_bytes().unwrap(), different.state_bytes().unwrap());
...
fn late_native_listeners_are_inactive_before_construction_and_handle_the_new_object_afterward() {
    ...
    let mut program = NativeSceneProgram::new(scene(71), vec![
        NativeSegment::Wait { duration: Some(0.5) },
        NativeSegment::defer(move |context| {
            ...
            context.event_dispatcher_mut().add_listener(EventListener::new(
                EventType::KeyPress, EventTarget::Global, move |event, _, _, stage| {
                    if matches!(event.payload, EventPayload::KeyPress { key: Key::Other(1001), .. }) {
    ...
    let key = || EventPayload::KeyPress { key: Key::Other(1001), modifiers: Modifiers::NONE };
