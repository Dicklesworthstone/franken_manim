...
use fmn_studio::{
    CURRENT_VERSION, Checkpoint, FrameHub, FramePayload, FrameStream, JournalReplay, ProtocolLimits,
    RequestEnvelope, ServiceError, SupervisorRequest, WorkerResponse, WorkerService,
    protocol_digest, read_response, serve_worker, write_request,
};

const NAME: &str = "DeferredScene";

fn initial() -> (Scene, Mob) {
    let mut scene = Scene::new(RuntimeConfig { fps: 8, ..RuntimeConfig::default() }, 71).unwrap();
    let points = [
...
fn program(builds: Rc<Cell<usize>>) -> Result<NativeSceneProgram, ServiceError> {
    ...
    NativeSceneProgram::new(scene, vec![
        ...
        NativeSegment::defer(move |context| {
            ...
            entry.buffer.write_range("fill_rgba", 0, &[1.0, 0.2, 0.1, 1.0].repeat(count));
            ...
            let mut tick = 0_u32;
            stage.add_updater(late, move |stage, target| {
                tick += 1;
                stage.shift_many(&[target], [f64::from(tick) * 0.01, 0.0, 0.0]);
            }, false)?;
            Ok(vec![NativeSegment::Wait { duration: Some(1.5) }])
        }),
    ], 16)
}
...
fn config(threads: usize) -> NativeWorkerConfig {
    let renderer = RetainedFrameRendererConfig {
        ...
        tiling: Tiling { macro_tile: 64, fine_tile: 8 }, engine: EngineIdentity::certified(), threads,
    };
    let mut config = NativeWorkerConfig::new(NAME, protocol_digest(b"deferred-build"),
        protocol_digest(b"deferred-source-and-inputs"), 17, 8, renderer);
...
fn seek(worker: &mut NativeSceneWorker, frame: i64) -> FrameStream {
    let WorkerResponse::Frame(frame) = worker.handle(SupervisorRequest::Scrub {
        scene: NAME.into(), frame,
    }).unwrap() else { panic!("real PNG frame") };
...
fn later_construction_changes_real_pixels_once_and_is_absent_before_its_boundary() {
    ...
    let mut worker = NativeSceneWorker::new(config(1), move || program(Rc::clone(&observed))).unwrap();
    ...
    assert_ne!(worker.program().unwrap().preview().stage().roots()[0], original);
    assert_eq!(seek(&mut worker, 5), after);
    worker.handle(SupervisorRequest::Inspect { scene: NAME.into() }).unwrap();
    assert_eq!(builds.get(), 1, "rendering and inspection must not invoke constructors");
...
fn restoring_before_and_after_construction_preserves_future_callback_motion() {
    for frame in [3, 6] {
        ...
        recovered.handle(SupervisorRequest::RestoreCheckpoint(checkpoint(&entry, 0))).unwrap();
...
fn journal_replay_crosses_the_dynamic_construction_boundary_with_exact_hashes() {
    ...
    journal.record(before.clone()).unwrap(); journal.record(after.clone()).unwrap();
    let expected = seek(&mut original, 10);
    let mut recovered = worker();
    recovered.handle(SupervisorRequest::RestoreCheckpoint(checkpoint(&before, 0))).unwrap();
    ...
    assert_eq!(response, WorkerResponse::ReplayComplete { from_entry: 1, state_hashes: vec![after.state_hash] });
...
fn late_geometry_and_mutable_updaters_are_identical_across_render_teams() {
    let expected = seek(&mut worker(), 10);
    for threads in [4, 16] {
        let mut worker = NativeSceneWorker::new(config(threads), || program(Rc::new(Cell::new(0)))).unwrap();
        assert_eq!(seek(&mut worker, 10), expected);
    }
}
...
fn failed_deferred_build_cannot_publish_a_successful_frame_or_checkpoint() {
    let mut worker = NativeSceneWorker::new(config(1), || {
        NativeSceneProgram::new(initial().0, vec![
            NativeSegment::Wait { duration: Some(0.5) },
            NativeSegment::edit(|_| Err(SceneError::InvalidState("late construction refused"))),
        ], 16)
    }).unwrap();
    let before = seek(&mut worker, 3);
    let failure = worker.handle(SupervisorRequest::Scrub { scene: NAME.into(), frame: 5 }).unwrap_err();
    assert!(failure.message.contains("late construction refused"));
    assert!(worker.program().is_none());
    assert!(worker.handle(SupervisorRequest::Play {
        scene: NAME.into(), command: studio_seek_command(NAME, 5).unwrap(),
    }).is_err());
    assert!(worker.journal_tail().is_empty());
    assert_eq!(worker.last_state_hash(), None);
    assert_eq!(seek(&mut worker, 3), before, "a fresh healthy prefix remains available");
}
...
fn a_deferred_panic_crosses_the_existing_worker_crash_boundary() {
    ...
    for (request_id, request) in [
        (1, SupervisorRequest::Hello { version: CURRENT_VERSION,
            supervisor_build: protocol_digest(b"parent"), max_frame_bytes: 1_000_000 }),
        (2, SupervisorRequest::Scrub { scene: NAME.into(), frame: 5 }),
    ] {
        write_request(&mut input, &RequestEnvelope { request_id, request }, limits).unwrap();
    }
    let mut output = Vec::new();
    let outcome = serve_worker(&mut worker, &mut Cursor::new(input), &mut output, limits).unwrap();
    assert!(matches!(outcome, fmn_studio::WorkerServeOutcome::Crashed(_)));
    let mut output = Cursor::new(output);
    assert!(matches!(read_response(&mut output, limits).unwrap().response, WorkerResponse::Hello { .. }));
