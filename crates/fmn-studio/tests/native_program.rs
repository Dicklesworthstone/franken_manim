...
fn waits_match_every_ordinary_native_capture_and_finish_once() {
    ...
    for frame in &expected.0 {
        ...
        assert_eq!(actual.materialize_stage().get_bounding_box(root),
...
fn input_runs_the_existing_editor_at_the_paused_native_time() {
    let (scene, root, calls) = source();
    let mut program = NativeSceneProgram::new(scene, vec![
        NativeSegment::Wait { duration: Some(1.0) },
    ], 30).expect("program");
    ...
    let count = calls.get();
    program.dispatch(EventPayload::MousePress {
        point: before.mid, button: MouseButton::Left, modifiers: Modifiers::PRIMARY,
    }).expect("select");
    program.dispatch(EventPayload::KeyPress {
    ...
    program.dispatch(EventPayload::KeyPress {
        key: Key::Character('z'), modifiers: Modifiers::PRIMARY,
    }).expect("undo");
...
fn clock_jumps_and_budget_overruns_do_not_execute_callbacks() {
    ...
    assert_eq!(program.advance_to(3).unwrap_err().code, WorkerErrorCode::InvalidRequest);
    ...
    assert!(program.state_bytes().is_ok(), "admission refusal must preserve the cursor");
}
...
fn invalid_segment_poisoning_prevents_reusing_partial_execution() {
    let (scene, _, _) = source();
    let mut program = NativeSceneProgram::new(scene, vec![
        NativeSegment::Wait { duration: Some(f64::NAN) },
    ], 10).expect("schedule is validated when opened");
    assert!(program.next_frame().is_err());
    assert!(program.state_bytes().is_err());
    assert!(program.dispatch(EventPayload::KeyPress {
        key: Key::ArrowRight, modifiers: Modifiers::NONE,
    }).is_err());
}
...
fn skip_range_and_presenter_scenes_refuse_before_adoption() {
    for config in [
        RuntimeConfig { skip_animations: true, ..RuntimeConfig::default() },
        RuntimeConfig { start_at_play: Some(1), ..RuntimeConfig::default() },
        RuntimeConfig { presenter_mode: true, ..RuntimeConfig::default() },
    ] {
