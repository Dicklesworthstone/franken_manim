...
impl Temp {
    fn new() -> Self {
        for _ in 0..128 {
            let path = std::env::temp_dir().join(format!("fmn-real-cargo-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
            if fs::create_dir(&path).is_ok() { return Self(path); }
        }
...
fn toolchain() -> (PathBuf, Vec<(String, String)>) {
    ...
    assert!(rustc.is_file(), "tests require the pinned toolchain's Cargo beside rustc");
    env.push(("RUSTC".into(), rustc.to_str().unwrap().into()));
    (cargo, env)
}

fn configure(root: &Path, cargo: PathBuf, env: Vec<(String, String)>, runner: &dyn ProcessRunner) -> CargoBuildConfig {
    let triple = CargoRebuildDriver::host_triple(runner, &cargo, &env).unwrap();
    let mut config = CargoBuildConfig::new(cargo, root.join("Cargo.toml"), "project-worker".into(),
        CargoTarget::Bin("project-worker".into()), triple, root.join("target"), root.join("images"));
    config.build_env = env; config.worker_argv = vec!["--worker".into()]; config
}

fn generate_lock(config: &CargoBuildConfig, runner: &dyn ProcessRunner) {
    let result = runner.run(&ProcessSpec {
        program: config.cargo.clone(), argv: vec!["generate-lockfile".into(), "--offline".into(),
            "--manifest-path".into(), config.manifest.to_str().unwrap().into()],
        env: config.build_env.clone(), cwd: None, stdin: None, timeout: Duration::from_secs(60), max_output_bytes: 2 * 1024 * 1024,
    }).unwrap();
    assert!(result.success(), "lockfile bootstrap: {}", String::from_utf8_lossy(&result.stderr));
}

#[test]
fn actual_compiler_failure_preserves_executable_and_fixed_source_builds_new_generation() {
    let root = Temp::new(); fs::create_dir(root.0.join("src")).unwrap();
    fs::write(root.0.join("Cargo.toml"), "[package]\nname='project-worker'\nversion='0.1.0'\nedition='2024'\n[workspace]\n").unwrap();
    let source = root.0.join("src/main.rs"); fs::write(&source, "fn main(){println!(\"first\");}\n").unwrap();
    let runner = Arc::new(StdProcessRunner::default()); let (cargo, env) = toolchain();
    let config = configure(&root.0, cargo, env, &*runner); generate_lock(&config, &*runner);
    let mut driver = CargoRebuildDriver::new(config, runner.clone()).unwrap();
    let first = driver.rebuild().unwrap();
    let execute = |path: PathBuf| {
        let result = runner.run(&ProcessSpec { program: path, argv: Vec::new(), env: Vec::new(),
            cwd: None, stdin: None, timeout: Duration::from_secs(10), max_output_bytes: 1024 }).unwrap();
    ...
    assert!(driver.rebuild().is_err()); assert!(!driver.last_outcome().unwrap().success());
    assert_eq!(execute(first.executable.clone()), b"first\n");
    fs::write(&source, "fn main(){println!(\"second\");}\n").unwrap();
    let second = driver.rebuild().unwrap(); assert_ne!(first.build_id, second.build_id);
...
fn worker_source(offset: f64) -> String {
    include_str!("fixtures/project_worker.rs").replace("const OFFSET: f64 = 0.0;", &format!("const OFFSET: f64 = {offset:?};"))
}

fn response(reply: SupervisorReply) -> WorkerResponse {
    match reply {
        SupervisorReply::Worker(WorkerResponse::Error { message, .. }) => panic!("worker refused: {message}"),
...
fn commit(supervisor: &mut Supervisor, frame: i64) {
    assert!(matches!(response(supervisor.request(SupervisorRequest::Play {
        scene: NAME.into(), command: studio_seek_command(NAME, frame).unwrap(),
    }, &|_| false).unwrap()), WorkerResponse::JournalSegment { .. }));
}

fn frame(supervisor: &mut Supervisor, index: i64) -> FrameStream {
    let WorkerResponse::Frame(frame) = response(supervisor.request(SupervisorRequest::Scrub {
        scene: NAME.into(), frame: index,
    }, &|_| false).unwrap()) else { panic!("expected actual rendered frame") };
    frame
}
fn png(frame: &FrameStream) -> &[u8] {
    let FramePayload::Pipe { bytes, .. } = &frame.payload else { panic!("inline PNG") }; bytes
}
...
fn http(host: &StudioHost, cap: &str, route: &str, body: &str) -> String {
    ...
    let bytes = std::thread::scope(|scope| {
        ...
        socket.set_read_timeout(Some(Duration::from_secs(330))).unwrap();
        let server = scope.spawn(|| host.serve_once().unwrap());
        write!(socket, "POST {route}?cap={cap} HTTP/1.1\r\nHost: {addr}\r\nOrigin: http://{addr}\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}", body.len()).unwrap();
        let mut bytes = Vec::new(); socket.take(256 * 1024).read_to_end(&mut bytes).unwrap();
        server.join().unwrap(); bytes
    });
...
fn real_project_rebuilds_replace_native_pixels_through_supervisor_and_http() {
    ...
    for name in ["fmn-core", "fmn-mobject", "fmn-render", "fmn-scene", "fmn-studio"] {
        let path = crates.join(name).to_str().unwrap().replace('\\', "\\\\").replace('"', "\\\"");
        manifest.push_str(&format!("{name}={{path=\"{path}\"}}\n"));
    }
    fs::write(root.0.join("Cargo.toml"), manifest).unwrap();
    let source = root.0.join("src/main.rs"); fs::write(&source, worker_source(0.0)).unwrap();
    let runner = Arc::new(StdProcessRunner::default()); let (cargo, env) = toolchain();
    let config = configure(&root.0, cargo, env, &*runner); generate_lock(&config, &*runner);
    ...
    let cache = Store::open(fs, Arc::clone(&clock), root.0.join("cache"), StoreConfig::default()).unwrap()
        .namespace("studio-replay", 1, NamespacePolicy { ceiling_bytes: Some(64 * 1024 * 1024) }).unwrap();
    ...
    supervisor.build_and_start(&mut driver).unwrap(); commit(&mut supervisor, 4);
    let first = frame(&mut supervisor, 4); let generation = supervisor.generation();
    let commands = supervisor.current_commands().unwrap();
    fs::write(&source, "invalid Rust\n").unwrap();
    assert!(supervisor.rebuild_and_restart(&mut driver, &commands, &|_| false).is_err());
    assert_eq!(supervisor.generation(), generation);
    assert_eq!(frame(&mut supervisor, 4), first, "failed compile must leave the old native worker usable");
    fs::write(&source, worker_source(0.8)).unwrap();
    let recovery = supervisor.rebuild_and_restart(&mut driver, &commands, &|_| false).unwrap();
    assert_eq!(supervisor.generation(), generation + 1);
    assert_eq!(recovery.plan.reuse, 0, "changed source is cold-reexecuted, not copied from stale checkpoints");
    for command in commands.into_iter().skip(recovery.plan.reuse) {
        response(supervisor.request(SupervisorRequest::Play { scene: NAME.into(), command }, &|_| false).unwrap());
    }
    let second = frame(&mut supervisor, 4); assert_ne!(png(&first), png(&second));

    // Use the real browser host's existing Restart action with the real compiler.
    let session = Arc::new(StudioWorkerSession::new(NAME, supervisor, Box::new(driver), Arc::new(|_| false)).unwrap());
    ...
    let token = CapabilityToken::new([7; 32]).unwrap(); let cap = token.try_expose_hex().unwrap();
    let host = StudioHost::bind(Arc::clone(&session), frames.clone(), token, clock, host_config).unwrap();
    ...
    let rebuilt = frames.latest().unwrap(); assert_eq!(rebuilt.frame_index, 4);
    assert_ne!(rebuilt.png, before.png);
    frames.close(); session.shutdown_worker();
    println!("OK: real Cargo builds, failed-edit worker preservation, source replacement, HTTP replay and changed native PNGs");
}
