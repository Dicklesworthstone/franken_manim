...
fn compiler_environment(cargo: &std::path::Path) -> Result<Vec<(String, String)>> {
    ...
    for key in ["PATH", "HOME", "CARGO_HOME", "RUSTUP_HOME", "RUSTUP_TOOLCHAIN",
        "TMPDIR", "TMP", "TEMP", "SYSTEMROOT", "WINDIR", "USERPROFILE", "APPDATA", "LOCALAPPDATA"] {
        if let Some(value) = std::env::var_os(key) {
            env.push((key.into(), value.into_string().map_err(|_| format!("{key} is not UTF-8"))?));
        }
    }
    let rustc = cargo.parent().ok_or("Cargo has no directory")?
        .join(format!("rustc{}", std::env::consts::EXE_SUFFIX));
    if !rustc.is_file() { return Err("select Cargo beside its rustc (for example: rustup which cargo)".into()); }
    env.push(("RUSTC".into(), rustc.to_str().ok_or("rustc path is not UTF-8")?.into()));
    Ok(env)
}
...
fn rebuild_http(addr: SocketAddr, cap: &str) -> Result<()> {
    ...
    write!(stream, "POST /api/restart?cap={cap} HTTP/1.1\r\nHost: {addr}\r\nOrigin: http://{addr}\r\nContent-Length: 0\r\nConnection: close\r\n\r\n")?;
    let mut bytes = Vec::new();
    stream.take(128 * 1024 + 1).read_to_end(&mut bytes)?;
    if bytes.len() > 128 * 1024 { return Err("rebuild response exceeded its byte limit".into()); }
    let text = String::from_utf8_lossy(&bytes);
    if !text.starts_with("HTTP/1.1 200 ") {
        return Err(format!("source rebuild refused; last published frame retained:\n{text}").into());
    }
    Ok(())
}
...
fn run() -> Result<()> {
    ...
    if !cargo.is_absolute() { return Err("Cargo must be an explicit absolute executable path".into()); }
    let manifest = PathBuf::from(&args[1]).canonicalize()?;
    let project = manifest.parent().ok_or("manifest has no parent")?.to_path_buf();
    ...
    let mut config = CargoBuildConfig::new(cargo, manifest.clone(), args[2].clone(), target,
        triple, target_dir.clone(), artifact_dir.clone());
    ...
    if args.len() == 6 { inputs.push(project.join("src")); }
    for input in args.iter().skip(6) { inputs.push(PathBuf::from(input).canonicalize()?); }
    let mut watcher = SourceWatch::new(inputs, vec![target_dir, artifact_dir], Duration::from_millis(300), WatchLimits::default())?;

    let clock: Arc<dyn Clock> = Arc::new(StdClock::new());
    let fs: Arc<dyn FileSystem> = Arc::new(VirtualFs::new());
    let cache = Store::open(fs, Arc::clone(&clock), project.join("target/fmn-project-cache"), StoreConfig::default())?
    ...
    let initial = (|| -> Result<_> {
        response(supervisor.request(SupervisorRequest::Play {
            scene: scene.clone(), command: studio_seek_command(&scene, 0)?,
        }, &|_| false)?)?;
        let WorkerResponse::Frame(frame) = response(supervisor.request(
            SupervisorRequest::Scrub { scene: scene.clone(), frame: 0 }, &|_| false,
        )?)? else { return Err("selected worker did not return its native initial frame".into()); };
        Ok(frame)
    })();
    let initial = match initial { Ok(frame) => frame, Err(error) => { supervisor.shutdown_worker(); return Err(error); } };
    let published_directory = builder.artifact_directory().map(std::path::Path::to_path_buf);
    let session = Arc::new(StudioWorkerSession::new(&scene, supervisor, Box::new(builder), Arc::new(|_| false))?);
    ...
    let host = StudioHost::bind(Arc::clone(&session), frames.clone(), token, Arc::clone(&clock), host_config)?;
        ...
        while !watch_stop.load(Ordering::Acquire) {
            match watcher.poll(clock.monotonic()) {
                ...
                Err(error) => {
                    ...
                    if message != last_error { eprintln!("Source watch: {message}"); last_error = message; }
                }
            }
...
fn main() {
    if let Err(error) = run() { eprintln!("native project Studio: {error}"); std::process::exit(1); }
}
