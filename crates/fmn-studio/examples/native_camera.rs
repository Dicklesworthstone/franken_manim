//! Native camera Studio: an animated surface, raster image and glow dot.
...
use fmn_scene::{AssetRead, Journal, PlayOverrides, RuntimeConfig, Scene};
...
fn surface() -> Result<Mobject> {
    ...
    for u in 0..SIDE {
        for v in 0..SIDE {
            ...
            // p + (-dz/dx, -dz/dy, 1): Lumen normalizes this differential.
...
fn program() -> std::result::Result<NativeSceneProgram, ServiceError> {
    ...
    NativeSceneProgram::new(
        ...
        vec![
            NativeSegment::Play {
                animations: vec![Box::new(rotate(surface, PI))],
...
fn config(build_id: ProtocolDigest) -> Result<NativeWorkerConfig> {
    let background = LinearRgba { r: 0.01, g: 0.015, b: 0.025, a: 1.0 };
    let renderer = RetainedFrameRendererConfig {
        frame: FrameConfig::new(
            Viewport { width: 320, height: 180 },
            ScreenMap { scale: 22.5, origin: [160.0, 90.0] },
            background,
        ),
    ...
    let mut policy = NativeWorkerConfig::new(
        NAME, build_id, protocol_digest(include_bytes!("native_camera.rs")), 301, 30, renderer,
    );
    let mut frame = CameraFrame::default();
    frame.set_euler_angles(Some(-0.4), Some(0.9), Some(0.0))?;
    policy.camera = Some(CameraConfig {
        resolution: (320, 180), fps: 30, background, frame,
...
fn run() -> Result<()> {
    ...
    if worker_mode {
        ...
        return match serve_worker(
            &mut worker, &mut std::io::stdin().lock(), &mut std::io::stdout().lock(), limits,
        )? {
    ...
    let mut supervisor = Supervisor::new(
        Box::new(StdWorkerLauncher::default()), Arc::clone(&clock), cache,
    ...
    let WorkerResponse::JournalSegment { journal, .. } = response(supervisor.request(
        SupervisorRequest::Play { scene: NAME.into(), command: studio_seek_command(NAME, 0)? },
        &|_| false,
    ...
    let result = if self_test {
        ...
        assert_ne!(first.png, moved.png, "the real surface animation must change pixels");
        ...
        println!("OK: native camera surface/image/dot PNGs, HTTP scrub/inspection and isolated restart identity");
        Ok(())
    } else {
        println!("{}", host.launch_url()?);
        eprintln!("Scrub the camera scene in Studio. Press Enter here to stop.");
