...
/// Unsupported TeX construct: `\substack` is a named, tier-tagged refusal
/// (fmd-math T2) that must surface verbatim through the e2e runner —
/// never a blank render, never a panic.
fn failure_tex_run(ctx: &mut RunCtx) -> Result<RunOutcome, ScenarioError> {
    let engine = &scene_goldens::corpus().tex;
    let source = r"\substack{a\\b}";
    match engine.typeset(fmn_tex::Mode::Math(fmd_math::Style::Text), source) {
        Ok(_) => Err(fail(
            "unsupported construct \\substack typeset successfully — the named refusal is gone",
        )),
        Err(error) => {
            let message = error.to_string();
            let is_math = matches!(error, TexError::Math(_));
            let names_construct = message.contains("substack");
