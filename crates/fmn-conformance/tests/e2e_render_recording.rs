...
use fmn_render::{Binning, FrameConfig, FrameJob, MonoTable, RenderPlan, ScreenMap, Tiling, Viewport};
...
fn render(stage: &Stage) -> Result<Vec<u8>, ScenarioError> {
    ...
    let mut binning = Binning::build(
        &plan, config.viewport, Tiling { macro_tile: 64, fine_tile: 8 }, config.map,
    ).map_err(error)?;
    binning.prune_occluded(&plan).map_err(error)?;
    let frame = FrameJob::new(&plan, &mono, &binning, config).map_err(error)?
        .render(1).map_err(error)?;
    fmn_render::engine::encode_frame(&frame).map_err(error)
}

fn invoke(ctx: &mut RunCtx) -> Result<RunOutcome, ScenarioError> {
    ctx.set_fps((8, 1));
    ctx.record_asset("render-recording.scene.source", include_bytes!("e2e_render_recording.rs"));
    ctx.event(LogEvent::new(spans::PREFLIGHT).field("history", 128u64));
    let mut stage = Stage::new();
    for _ in 0..128 {
        stage.add(Mobject::from_points(&[[42.0, 42.0, 0.0]]));
    }
    let root = stage.add(Circle::new().radius(0.6)
        .style(Style::default().color(BLUE_C).fill_opacity(0.6)).build());
    ...
    if compact.frame_count != 1 || full.bytes.len() <= compact.bytes.len() {
        return Err(ScenarioError::new("render projection did not remove captured arena history"));
    }
    if !stage.contains(root) || before != stage.snapshot().to_bytes().map_err(error)? {
        return Err(ScenarioError::new("recording mutated the live stage or its ordinary snapshot"));
    }
    ctx.event(LogEvent::new("recording.projection")
    ...
    if expected != ordinary_frame || expected != replay_frame {
        return Err(ScenarioError::new("render-only replay differs from the observed/full-state frame"));
    }
    ...
    if repeat.finish_with_max_bytes(compact.bytes.len()).map_err(error)?.bytes != compact.bytes {
        return Err(ScenarioError::new("exact-budget render-only replay is not deterministic"));
    }
    ctx.event(LogEvent::new(spans::RENDER_FRAME).field("frames", 1u64).field("equal", true));
...
fn scenario() -> ScenarioSpec {
    ScenarioSpec::new(
        ...
        Assertion::FileInventory(vec!["render-only.fmtl".to_owned(), "render-only.frame".to_owned()]),
        Assertion::NdjsonSchema,
    ]).logs(vec![
        LogExpect::span_present("recording.projection", vec![FieldPred::str_eq("mode", "render-only")]),
