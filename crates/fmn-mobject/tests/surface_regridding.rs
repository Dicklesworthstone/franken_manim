...
fn grid(stage: &mut Stage, shape: (usize, usize)) -> Mob {
    let schema = RecordSchema::new(
        &[("point", 3), ("d_normal_point", 3), ("rgba", 4), ("im_coords", 2), ("custom", 1)],
        &["point"],
    ...
    stage.add(Mobject::from_buffer(buffer).with_render_primitive(
...
fn assert_grid(stage: &Stage, mob: Mob, shape: (usize, usize)) {
    ...
    assert_eq!(entry.render_primitive(), RenderPrimitive::SurfaceGrid { resolution: shape });
...
fn geometry_rebuild_preserves_paint_uvs_custom_fields_and_native_owners() {
    ...
    let update = SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), shape, &points, &normals).unwrap();
...
fn equal_shape_geometry_rebuild_keeps_live_views_and_frozen_snapshots() {
    ...
    let update = SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), (2, 3), &[2.0; 18], &[3.0; 18]).unwrap();
    stage.apply_surface_grid_update(mob, update).unwrap();
    assert!(view.is_attached_to(&stage.get(mob).unwrap().buffer));
...
fn prepared_geometry_refuses_subsequent_retained_placement_or_record_edits() {
    for edit_placement in [false, true] {
        let mut stage = Stage::new();
        let mob = grid(&mut stage, (2, 3));
        let update = SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), (3, 4), &[2.0; 36], &[3.0; 36]).unwrap();
        if edit_placement {
            stage.shift(mob, [1.0, 0.0, 0.0]);
        } else {
            stage.get_mut(mob).unwrap().buffer.write(0, "rgba", &[0.0; 4]);
        }
        ...
        if edit_placement {
            ...
            assert_eq!(stage.get(mob).unwrap().buffer.read(0, "rgba").unwrap(), [0.0; 4]);
        }
    }
}
...
fn geometry_admission_refuses_invalid_lengths_samples_and_budgets_before_mutation() {
    ...
    for (shape, points, normals) in [
        ...
        assert!(SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), shape, &points, &normals).is_err());
...
fn geometry_rebuild_bakes_auxiliary_pointlikes_but_never_color_or_texture_lanes() {
    ...
    let schema = RecordSchema::new(
        ...
        &["point"], &["point", "d_normal_point", "hint"],
    ).unwrap();
    ...
    let mob = stage.add(Mobject::from_buffer(buffer).with_render_primitive(
        RenderPrimitive::SurfaceGrid { resolution: (2, 2) }));
    stage.shift(mob, [4.0, -1.0, 2.0]);
    let update = SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), (3, 3), &[0.0; 27], &[0.1; 27]).unwrap();
    stage.apply_surface_grid_update(mob, update).unwrap();
    for row in 0..9 {
        assert_eq!(stage.get(mob).unwrap().buffer.read(row, "hint").unwrap(), [5.0, 1.0, 5.0]);
        assert_eq!(stage.get(mob).unwrap().buffer.read(row, "rgb").unwrap(), [0.25, 0.5, 1.0]);
    }
}
...
fn auxiliary_world_overflow_refuses_geometry_rebuild_without_resetting_placement() {
    ...
    let mob = stage.add(Mobject::from_buffer(buffer).with_render_primitive(
        RenderPrimitive::SurfaceGrid { resolution: (2, 2) }));
    stage.set_placement(mob, Placement::from_translation([f64::MAX, 0.0, 0.0])).unwrap();
    let placement = stage.placement(mob);
    assert!(SurfaceGridUpdate::for_geometry(stage.get(mob).unwrap(), (3, 3), &[0.0; 27], &[0.1; 27]).is_err());
