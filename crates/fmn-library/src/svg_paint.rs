...
use fmn_geom::{
    ArcLengthTable, BooleanError, BooleanLimits, BooleanOperation, BooleanOptions,
    FillRule, GeomError, QuadPath, path_boolean,
};
...
impl From<GeomError> for SvgPaintError {
    fn from(error: GeomError) -> Self { Self::Geometry(error) }
}
impl From<BooleanError> for SvgPaintError {
    fn from(error: BooleanError) -> Self { Self::Boolean(error) }
}
...
impl Budget {
    fn points(&mut self, count: usize) -> Result<(), SvgPaintError> {
        self.points = self.points.checked_add(count)
...
impl SvgPaintOverrides {
    fn apply(self, mut style: Style) -> Result<Style, SvgPaintError> {
        for color in [self.fill_color, self.stroke_color].into_iter().flatten() {
            if [color.r, color.g, color.b].iter().any(|v| !v.is_finite() || !(0.0..=1.0).contains(v)) {
                return Err(SvgPaintError::Invalid("RGB components must be finite and in [0, 1]"));
            }
        }
        for alpha in [self.fill_opacity, self.stroke_opacity].into_iter().flatten() {
            if !alpha.is_finite() || !(0.0..=1.0).contains(&alpha) {
                return Err(SvgPaintError::Invalid("opacities must be finite and in [0, 1]"));
            }
        }
        if let Some(width) = self.stroke_width {
            if !width.is_finite() || !(0.0..=f64::from(f32::MAX)).contains(&width) {
                return Err(SvgPaintError::Invalid("stroke width must be nonnegative and f32-representable"));
            }
            style.stroke_width = width;
        }
        if let Some(color) = self.fill_color { style.fill_color = color; }
        if let Some(color) = self.stroke_color { style.stroke_color = color; }
        if let Some(alpha) = self.fill_opacity { style.fill_opacity = alpha; }
        if let Some(alpha) = self.stroke_opacity { style.stroke_opacity = alpha; }
        Ok(style)
    }
}
...
pub fn svg_document_with_paints(
    ...
    let mut budget = Budget {
        points: 0, pieces: 0, steps: 0, boolean: BooleanLimits::default(),
    };
...
pub fn svg_mobject_with_paints(
    bytes: &[u8], overrides: SvgPaintOverrides,
) -> Result<VMobject, SvgPaintError> {
...
fn prepare_shape(
    ...
    // Later styling must reveal the same filled set/dashes, not a solid path.
    {
        let fill = if even_odd {
            ...
            budget.boolean.max_flattened_segments = budget.boolean.max_flattened_segments.saturating_sub(used.flattened_segments);
            budget.boolean.max_pair_tests = budget.boolean.max_pair_tests.saturating_sub(used.pair_tests);
            budget.boolean.max_intersections = budget.boolean.max_intersections.saturating_sub(used.intersections);
            budget.boolean.max_classification_tests = budget.boolean.max_classification_tests.saturating_sub(used.classification_tests);
            budget.boolean.max_output_segments = budget.boolean.max_output_segments.saturating_sub(used.output_segments);
            result.path
...
fn dash_paths(shape: &SvgShape, budget: &mut Budget) -> Result<Vec<QuadPath>, SvgPaintError> {
    ...
    if raw.len() > MAX_SVG_DASH_PIECES || raw.iter().any(|v| !v.is_finite() || *v < 0.0)
        || !shape.style.stroke_dashoffset.is_finite() {
        return Err(SvgPaintError::Invalid("dash pattern must be bounded, finite and nonnegative"));
    }
    let mut pattern = raw.clone();
    if pattern.len() % 2 == 1 { pattern.extend_from_slice(raw); }
    let period: f64 = pattern.iter().sum();
    if !period.is_finite() || period <= 0.0 {
        return Err(SvgPaintError::Invalid("dash period must be positive and finite"));
    }
    if pattern.iter().step_by(2).any(|&v| v == 0.0) {
        return Err(SvgPaintError::Invalid("zero-length painted dashes require dot-cap support; use positive dash lengths"));
    }
    ...
    for points in shape.path.subpaths() {
        ...
        if !length.is_finite() { return Err(SvgPaintError::Invalid("nonfinite stroke length")); }
        ...
        for &(a, b) in &intervals {
            let alpha = |distance: f64| {
                let (curve, t) = table.curve_and_t_at(&path, distance / length)
                    .ok_or(SvgPaintError::Invalid("dash distance could not be resolved"))?;
                Ok::<f64, SvgPaintError>((curve as f64 + t) / path.num_curves() as f64)
            };
            let (points, _, _) = QuadPath::partial_points(path.points(), alpha(a)?, alpha(b)?)
                .ok_or(SvgPaintError::Invalid("dash interval could not be extracted"))?;
            pieces.push(QuadPath::from_points(points)?);
        }
        ...
        if pieces.len() > 1 && points.first() == points.last()
