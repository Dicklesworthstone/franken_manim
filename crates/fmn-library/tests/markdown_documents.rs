...
use fmn_library::markdown::{Markdown, MarkdownMathOptions, MAX_MARKDOWN_BYTES};
...
fn book() -> FontBook { FontBook::bundled().unwrap() }
fn engine() -> TexEngine { TexEngine::new("fmd-math/pack/default", None).unwrap() }
fn canonical(v: &VMobject) -> Vec<[f64;3]> {
    fn append(v: &VMobject, points: &mut Vec<[f64;3]>) {
        points.extend_from_slice(v.points());
        for c in v.children() { append(c, points); }
    }
    let mut points = Vec::new(); append(v, &mut points);
    if let Some(origin) = points.first().copied() {
        for p in &mut points { for i in 0..3 { p[i] -= origin[i]; } }
    }
    points
}
fn same_shape(a: &VMobject, b: &VMobject) {
    let (a,b) = (canonical(a),canonical(b)); assert_eq!(a.len(),b.len());
    for (a,b) in a.iter().zip(&b) {
        for i in 0..3 { assert!((a[i]-b[i]).abs() < 1e-10, "{a:?} != {b:?}"); }
    }
}
...
fn inline_math_preserves_tex_underscores_and_uses_native_layout() {
    ...
    let doc = Markdown::new(r"before $x_i+y_j$ after").build_with_math(&book,&engine).unwrap();
...
fn display_and_fenced_math_are_exact_native_formulas() {
    let (book, engine) = (book(),engine());
    for source in ["$$\\frac{1}{x^2}$$", "```math\n\\frac{1}{x^2}\n```", "```tex\n\\frac{1}{x^2}\n```"] {
        let doc = Markdown::new(source).build_with_math(&book,&engine).unwrap();
        same_shape(&doc.vmob,&Tex::new(r"\frac{1}{x^2}").display().build(&engine).unwrap().vmob);
    }
}
...
fn nested_blocks_keep_all_content_and_do_not_overlap() {
    let source="# Title\n\n- outer\n  - nested\n\n> ## Quote\n>\n> body\n\n| a | b |\n|---|---|\n| 1 | 2 |\n";
    let doc=Markdown::new(source).block_gap(0.3).build_with_math(&book(),&engine()).unwrap();
    assert_eq!(doc.blocks.iter().map(|b|b.kind).collect::<Vec<_>>(),["heading","list","quote","table"]);
    ...
    assert!(canonical(&doc.blocks[1].vmob).len() > canonical(&Markdown::new("- outer").build(&book()).unwrap().vmob).len());
...
fn wrapping_preserves_atoms_and_separates_tall_math_lines() {
    ...
    let wide=Markdown::new(source).build_with_math(&book,&engine).unwrap();
    let narrow=Markdown::new(source).build_with_math_options(&book,&engine, MarkdownMathOptions { line_width: Some(1.5), ..Default::default() }).unwrap();
    assert!(narrow.vmob.length_over_dim(1)>wide.vmob.length_over_dim(1));
    assert_eq!(wide.blocks[0].vmob.children().len(),narrow.blocks[0].vmob.children().len());
    for (a,b) in wide.blocks[0].vmob.children().iter().zip(narrow.blocks[0].vmob.children()) { same_shape(a,b); }
}
...
fn original_unicode_source_ranges_survive_math_protection() {
    ...
    for pair in doc.blocks.windows(2) { assert!(pair[0].byte_range.1<=pair[1].byte_range.0); }
}
...
fn source_and_layout_limits_refuse_before_unbounded_expansion() {
    ...
    assert!(Markdown::new("x").build_with_math_options(&book,&engine(), MarkdownMathOptions { line_width: Some(f64::NAN), ..Default::default() }).is_err());
    assert!(Markdown::new("x").font_size(-1.0).build(&book).is_err());
    assert!(Markdown::new(&"$x$ ".repeat(257)).build_with_math(&book,&engine()).is_err());
    assert!(Markdown::new("![missing](https://example.invalid/image.png)").build_with_math(&book,&engine()).is_err());
    assert!(Markdown::new("$x$").build(&book).is_ok(), "the text-only path remains literal");
}
