...
fn invisible_channels_retain_geometry_for_later_styling() {
    ...
    let built=svg_mobject_with_paints(source,SvgPaintOverrides{stroke_width:Some(0.),..Default::default()}).unwrap();
    let layers=built.children()[0].children();
    assert!((area(&layers[0]).abs()-64.).abs()<1e-8);
    assert!(layers.len()>3);
    assert!(layers.iter().all(|layer| layer.style().fill_opacity==0. && layer.style().stroke_opacity==0.));
}
