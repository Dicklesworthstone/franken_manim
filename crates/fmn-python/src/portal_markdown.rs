...
fn _build_math_markdown<'py>(
    slf: &Bound<'py, BridgeMobject>, factory: &Bound<'py, PyAny>, source: &str,
    font_size: f64, line_width: Option<f64>, block_gap: f64, code_style: &str,
    color: Option<&Bound<'py, PyAny>>,
) -> PyResult<BuiltDocument<'py>> {
    if slf.try_borrow()?.engine.is_some() {
        return Err(PyValueError::new_err("mathematical Markdown construction requires a detached receiver"));
    }
    if source.len() > MAX_MARKDOWN_BYTES {
        return Err(PyValueError::new_err("Markdown source exceeds 32768 UTF-8 bytes"));
    }
    ...
    let color = color.map(srgb_from_py).transpose()?.unwrap_or(fmn_core::constants::WHITE);
    let builder = Markdown::new(source).font_size(font_size).block_gap(block_gap).theme(theme);
    let built = with_font_book(|book| crate::with_tex_engine(|engine| {
