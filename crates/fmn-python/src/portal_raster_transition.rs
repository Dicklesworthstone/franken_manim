...
use super::{RasterImage, _replace_raster_image};
...
fn policy(sampler: ImageSampler) -> SamplerPolicy {
    ...
    SamplerPolicy { wrap_u: wrap(sampler.wrap_u), wrap_v: wrap(sampler.wrap_v) }
}
...
fn decode(image: &ImageResource) -> PyResult<Arc<Texture>> {
    ...
    Texture::from_rgba8(image.width(), image.height(), image.pixels(), encoding)
        .map(Arc::new).map_err(native_error)
}

fn shape(left: &ImageResource, right: &ImageResource) -> PyResult<(u32, u32)> {
    let size = (left.width().max(right.width()), left.height().max(right.height()));
    if u64::from(size.0) * u64::from(size.1) > MAX_PIXELS {
        return Err(PyValueError::new_err("raster transition output exceeds its 16M-pixel budget"));
    }
    Ok(size)
}
...
impl Plane {
    fn sample(&self, alpha: f64, sampler: ImageSampler) -> PyResult<ImageResource> {
        ...
        for y in 0..height {
            for x in 0..width {
                let uv = [(f64::from(x) + 0.5) / f64::from(width),
                ...
                let color = PremulRgba { r: lerp(a.r, b.r), g: lerp(a.g, b.g),
                                        b: lerp(a.b, b.b), a: lerp(a.a, b.a) }.unpremultiply();
...
impl RasterTransition {
    ...
    fn new(target: &Bound<'_, BridgeMobject>, image: &RasterImage,
           dark_image: Option<&RasterImage>) -> PyResult<Self> {
        let (start, primitive) = with_stage(target, |stage, mob| {
            let entry = stage.get(mob)
                .ok_or_else(|| StaleHandleError::new_err("raster transition target is stale"))?;
            let start = entry.image_resource().cloned()
                .ok_or_else(|| PyValueError::new_err("raster transition requires a native image material"))?;
            Ok::<_, PyErr>((start, entry.render_primitive()))
        })??;
        let mut end = image.resource.clone();
        if let Some(dark) = dark_image {
            end = end.with_dark_image(dark.resource.clone()).map_err(native_error)?;
        }
        if !matches!(primitive, RenderPrimitive::ImageQuad | RenderPrimitive::SurfaceGrid { .. }
        ...
        let dark_shape = if has_dark {
            Some(shape(start.dark_image().unwrap_or(&start), end.dark_image().unwrap_or(&end))?)
        } else { None };
        ...
        target.py().detach(move || {
            ...
            let dark = if let Some(size) = dark_shape {
                Some(Plane {
                    left: start.dark_image().map_or_else(|| Ok(left.clone()), decode)?,
    ...
    fn sample(&self, py: Python<'_>, alpha: f64) -> PyResult<RasterImage> {
        if !alpha.is_finite() {
            return Err(PyValueError::new_err("raster transition alpha must be finite"));
        }
...
mod tests {
    ...
    fn production_raster_transition_kernel_acceptance() {
        crate::with_python_test_module("raster transition", |py, _module, globals| {
            let text = std::ffi::CString::new(include_str!("../tests/raster_transition_kernel.py")).unwrap();
            py.run(text.as_c_str(), Some(globals), Some(globals))
                .inspect_err(|error| error.print(py)).expect("native raster transition colors and resources");
        });
    }
}
