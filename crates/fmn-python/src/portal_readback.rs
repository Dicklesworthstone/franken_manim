...
use fmn_render::{EngineIdentity, FrameConfig, RetainedFrameRenderer, RetainedFrameRendererConfig, ScreenMap, Tiling, Viewport};
use pyo3::prelude::*;
use pyo3::types::{PyBytes, PyList};

use crate::{BridgeMobject, PyCameraCore, PyCameraFrameCore, PyRuntimeError, PyValueError, StaleHandleError, native_error, stage_error};
...
fn graph(children: &Bound<'_, PyList>, roots: &Bound<'_, PyList>, count: usize) -> PyResult<(Vec<Vec<usize>>, Vec<usize>)> {
    if count > MAX_NODES || children.len() != count || roots.len() > MAX_NODES {
        return Err(PyValueError::new_err("camera capture family exceeds its node budget or has invalid adjacency"));
    }
    ...
    for row in children.iter() {
        ...
        edges = edges.checked_add(row.len()).ok_or_else(|| PyValueError::new_err("camera capture edge count overflow"))?;
        if edges > MAX_EDGES {
            return Err(PyValueError::new_err("camera capture family exceeds its edge budget"));
        }
        let mut values = Vec::with_capacity(row.len());
        for value in row.iter() {
            let child: usize = value.extract()?;
            if child >= count {
                return Err(PyValueError::new_err("camera capture child index is out of range"));
            }
            incoming[child] += 1;
            values.push(child);
        }
    ...
    let mut ready: Vec<usize> = incoming.iter().enumerate().filter_map(|(index, &degree)| (degree == 0).then_some(index)).collect();
    let mut cursor = 0;
    ...
    if ready.len() != count {
        return Err(PyValueError::new_err("camera capture family contains a cycle"));
    }
    let roots: Vec<usize> = roots.extract()?;
    if roots.iter().any(|&root| root >= count) {
        return Err(PyValueError::new_err("camera capture root index is out of range"));
    }
    Ok((adjacency, roots))
}

fn copy_entry(source: &Stage, mob: Mob, target: &mut Stage, bytes: &mut usize) -> PyResult<Mob> {
    let entry = source.get(mob).ok_or_else(|| StaleHandleError::new_err("camera capture source no longer resolves"))?;
    *bytes = entry.buffer.len().checked_mul(entry.buffer.schema().stride())
...
impl CameraCapture {
    ...
    fn new(
        core: &Bound<'_, PyCameraCore>, frame: &Bound<'_, PyCameraFrameCore>,
        background: [f64; 4], light: [f64; 3], objects: &Bound<'_, PyList>,
        children: &Bound<'_, PyList>, roots: &Bound<'_, PyList>, threads: usize,
    ) -> PyResult<Self> {
        if !(1..=96).contains(&threads) {
            return Err(PyValueError::new_err("camera capture threads must lie in 1..=96"));
        }
        if !background.iter().all(|value| value.is_finite() && (0.0..=1.0).contains(value)) {
            return Err(PyValueError::new_err("camera background RGBA must be finite in 0..=1"));
        }
        ...
        *camera.frame_mut() = frame.try_borrow()?.frame.clone();
        ...
        if u64::from(width) * u64::from(height) > MAX_PIXELS {
            return Err(PyValueError::new_err("camera capture exceeds the 16M-pixel budget"));
        }
        let background = Srgb { r: background[0], g: background[1], b: background[2] }.to_linear(background[3]);
        camera.set_background(background).map_err(native_error)?;
        camera.set_light_source_position(light).map_err(native_error)?;
        ...
        for object in objects.iter() {
            ...
            let handle = if let (Some(engine), Some(mob)) = (&object.engine, object.mob) {
                let scene = engine.scene.try_borrow().map_err(|_| PyRuntimeError::new_err("camera capture cannot read a mutably borrowed Scene"))?;
                ...
                return Err(StaleHandleError::new_err("camera capture source is uninitialized"));
            };
            handles.push(handle);
        }
        for (parent, children) in adjacency.iter().enumerate() {
            for &child in children {
                stage.attach(handles[parent], handles[child]).map_err(stage_error)?;
            }
        }
        ...
        let config = FrameConfig::new(
            ...
            ScreenMap { scale: f64::from(height) / fmn_core::constants::FRAME_HEIGHT,
                origin: [f64::from(width) / 2.0, f64::from(height) / 2.0] }, background,
        ).with_aa_policy(camera.aa_policy());
        let mut renderer = RetainedFrameRenderer::new(RetainedFrameRendererConfig {
            frame: config, tiling: Tiling::default(), engine: EngineIdentity::fast(), threads,
        }).map_err(native_error)?;
        // Camera projection is essential even for 2D: this route owns the
        // +Y-up scene to top-row-first framebuffer orientation.
        renderer.render_with_camera(&stage, &camera).map_err(native_error)?;
        let mut rgba = FrameBuffer::new(FrameLayout::tight(PixelFormat::Rgba8, width, height).map_err(native_error)?);
...
mod tests {
    ...
    fn production_camera_readback_acceptance_suite() {
        crate::with_python_test_module("camera readback", |py, _module, globals| {
            let source = std::ffi::CString::new(include_str!("../tests/camera_readback.py")).unwrap();
