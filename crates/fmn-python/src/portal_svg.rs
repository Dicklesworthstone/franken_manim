...
impl SvgSession {
    pub(super) fn new(
        ...
        let background = fmn_core::color::Srgb::from_hex(&config.camera.background_color)
            .map_err(native_error)?.to_linear(config.camera.background_opacity);
        let camera = Camera::new(CameraConfig {
            resolution: (width, height), fps, background, ..CameraConfig::default()
        ...
        Ok((Self {
            destination, camera, threads, timeline: OutputTimeline::new(true),
        }, runtime))
    }
    ...
    pub(super) fn bind_camera(
        ...
        *self.camera.frame_mut() = frame;
        if let Some(background) = background {
            self.camera.set_background(background).map_err(camera_error)?;
        }
        Ok(())
    }
    ...
    pub(super) fn finish(self, scene: &Scene) -> PyResult<(PortalArtifactReport, String, usize)> {
        ...
        Ok((PortalArtifactReport {
            path: report.path, frame_count: 1, bytes: report.bytes, digest: report.digest,
            invocations: Vec::new(), audio_inputs: Vec::new(),
        }, "native-svg".to_owned(), self.threads))
    }
}

fn unsupported(detail: &str) -> PyErr {
    CapabilityError::new_err(format!("SVG export: {detail}; use PNG for raster/3D rendering"))
}
...
fn validate_stage(stage: &Stage) -> PyResult<()> {
    ...
    for root in stage.roots() {
        for mob in stage.family(*root) {
            let entry = stage.get(mob).ok_or_else(|| unsupported("stale scene member"))?;
            if entry.buffer.is_empty() { continue; }
            shapes = shapes.checked_add(1).ok_or_else(|| unsupported("shape budget exceeded"))?;
            points = points.checked_add(entry.buffer.len()).ok_or_else(|| unsupported("point budget exceeded"))?;
            ...
            if uniforms.depth_test || uniforms.shading != [0.0; 3]
                || uniforms.clip_planes != [[0.0; 4]; 4]
            {
                return Err(unsupported("depth, lighting and user clip planes require Lumen"));
            }
            for (name, width) in [("fill_rgba", 4), ("stroke_rgba", 4), ("stroke_width", 1)] {
                ...
                if values.len() != entry.buffer.len() * width || !values.iter().all(|v| v.is_finite()) {
                    return Err(unsupported("invalid vector style records"));
                }
                if values.chunks_exact(width).any(|row| row != &values[..width]) {
...
fn scene_document(stage: &Stage, camera: &Camera) -> PyResult<String> {
    ...
    for root in stage.roots() {
        for mob in stage.family(*root) {
            if !seen.insert(mob) { continue; }
            let points = stage.get_points(mob).ok_or_else(|| unsupported("stale point records"))?;
            if points.is_empty() { continue; }
            let uniforms = *stage.uniforms(mob).ok_or_else(|| unsupported("stale uniforms"))?;
            let mut weight = None;
            let mut mapped = Vec::new();
            mapped.try_reserve_exact(points.len()).map_err(native_error)?;
            for point in points {
                ...
                if !clip.clip.iter().all(|v| v.is_finite()) || w <= 0.0
                    || clip.clip[2].abs() > w || weight.is_some_and(|old| old != w)
                {
    ...
    stage_svg_document_with_background(&projected, f64::from(width), f64::from(height),
        f64::from(width), f64::from(height), Some(camera.background()))
        .map_err(native_error)
}
...
pub fn run_portal_gauntlet_svg() -> Result<PortalSvgReport, String> {
    crate::with_python_test_module("native SVG output", |py, _module, globals| {
        globals.set_item("__file__", concat!(env!("CARGO_MANIFEST_DIR"), "/tests/svg_output.py"))
            .map_err(|error| error.to_string())?;
        let source = CString::new(include_str!("../tests/svg_output.py")).map_err(|error| error.to_string())?;
        py.run(source.as_c_str(), Some(globals), Some(globals))
            .inspect_err(|error| error.print(py)).map_err(|error| error.to_string())?;
        let get = |name: &str| globals.get_item(name).map_err(|error| error.to_string())?
            .ok_or_else(|| format!("missing SVG witness: {name}"));
        Ok(PortalSvgReport {
            document: get("_svg_document")?.extract().map_err(|error: PyErr| error.to_string())?,
            thread_counts: get("_svg_thread_counts")?.extract().map_err(|error: PyErr| error.to_string())?,
            failure_paths: get("_svg_failure_paths")?.extract().map_err(|error: PyErr| error.to_string())?,
        })
    })
}
...
mod tests {
    ...
    fn svg_projection_preserves_native_geometry_without_mutating_the_scene() {
        ...
        let mut camera = Camera::new(CameraConfig { resolution: (160, 90), ..CameraConfig::default() }).unwrap();
