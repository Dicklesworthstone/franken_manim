...
fn grid(entry: &fmn_mobject::Entry) -> PyResult<Option<(usize, usize)>> {
    ...
    for key in ["point", "d_normal_point"] {
        if !entry.buffer.schema().fields().iter().any(|f| f.name == key && f.width == 3) {
            return Err(PyValueError::new_err("surface geometry requires point and normal-control vec3 fields"));
        }
    }
    Ok(Some(resolution))
}
...
fn _surface_grid_resolution(target: &Bound<'_, BridgeMobject>) -> PyResult<Option<(usize, usize)>> {
    with_stage(target, |stage, mob| {
        grid(stage.get(mob).ok_or_else(|| StaleHandleError::new_err("surface is stale"))?)
    })?
}
...
fn world_column(entry: &fmn_mobject::Entry, key: &str) -> PyResult<Vec<f32>> {
    let input = entry.buffer.read_column(key).ok_or_else(|| PyValueError::new_err("surface field is missing"))?;
    let mut output = Vec::with_capacity(input.len());
    for point in input.as_chunks::<3>().0 {
        let world = entry.placement().apply_point(point.map(f64::from));
        if world.iter().any(|v| !v.is_finite() || v.abs() > f64::from(f32::MAX)) {
            return Err(PyValueError::new_err("surface geometry must be finite and f32-representable"));
        }
...
#[pyfunction]
fn _copy_surface_geometry(target: &Bound<'_, BridgeMobject>, source: &Bound<'_, BridgeMobject>) -> PyResult<()> {
    ...
    let geometry = with_stage(source, |stage, mob| -> PyResult<Geometry> {
        ...
        Ok(Geometry {
            resolution: grid(entry)?.ok_or_else(|| PyTypeError::new_err("source must be a native UV surface"))?,
            points: world_column(entry, "point")?,
            normal_points: world_column(entry, "d_normal_point")?,
        })
    })??;
    with_stage(target, |stage, mob| -> PyResult<()> {
        let entry = stage.get(mob).ok_or_else(|| StaleHandleError::new_err("target surface is stale"))?;
        if grid(entry)? != Some(geometry.resolution) {
            return Err(PyValueError::new_err("live surface regeneration requires matching native UV topology"));
        }
        ...
        let entry = stage.get_mut(mob).ok_or_else(|| StaleHandleError::new_err("target surface is stale"))?;
        entry.buffer.write_range("point", 0, &geometry.points);
        entry.buffer.write_range("d_normal_point", 0, &geometry.normal_points);
        Ok(())
    })?
}
...
mod tests {
    ...
    fn live_surface_regeneration_and_native_output() {
        crate::with_python_test_module("live surface geometry", |py, _module, globals| {
            let code = std::ffi::CString::new(include_str!("../tests/surface_geometry.py")).unwrap();
            py.run(code.as_c_str(), Some(globals), Some(globals))
                .inspect_err(|error| error.print(py)).unwrap();
            globals.get_item("run_surface_geometry_acceptance").unwrap().unwrap()
