...
fn with_capture<T>(scene: &Bound<'_, PyScene>, f: impl FnOnce(&mut Capture) -> Result<T, ServiceError>) -> Result<T, ServiceError> {
    ...
    let Some(PortalRenderSession::Preview(capture)) = slot.as_mut() else {
        return Err(failure("live Studio capture ownership was lost; reload the scene"));
    };
    f(capture)
}
...
fn refresh(scene: &Bound<'_, PyScene>) -> PyResult<()> {
    ...
    with_capture(scene, |capture| { capture.live_refresh = true; Ok(()) }).map_err(native_error)?;
    let render = Arc::clone(&scene.try_borrow()?.render);
    let mut sink = PortalSceneSink { render, ..PortalSceneSink::default() };
    let result = engine.borrow_mut().show(&mut sink).map_err(native_error);
    with_capture(scene, |capture| { capture.live_refresh = false; Ok(()) }).map_err(native_error)?;
    result
}
...
impl LiveWorker {
    fn new(scene: &Bound<'_, PyScene>) -> PyResult<Self> {
        let empty = with_capture(scene, |capture| {
            if capture.live { return Err(failure("live Studio is already serving this scene")); }
            Ok(capture.is_empty())
        }).map_err(native_error)?;
        if empty { refresh(scene)?; }
        ...
        refresh(scene)?;
        let state_hash = with_capture(scene, |c| Ok(c.recorded.last_state_hash())).map_err(native_error)?;
        Ok(Self { scene: scene.clone().unbind(), name, build, state_hash, failed: None })
    }

    fn handle_attached(&mut self, py: Python<'_>, request: SupervisorRequest) -> Result<WorkerResponse, ServiceError> {
        let scene = self.scene.bind(py);
        let result = match request {
            SupervisorRequest::Play { scene: name, command } if command.kind == CommandKind::Input => {
                if let Some(error) = &self.failed {
                    return Err(failure(format!("live input is frozen after callback failure; reload: {error}")));
                }
                let input = with_capture(scene, |c| c.recorded.prepare_live_input(&name, &command))?;
                ...
                if let Err(error) = dispatched {
                    ...
                    let _ = with_capture(scene, |c| { c.recorded.disable_live_input(); Ok(()) });
                    return Err(failure(format!("live callback failed; input frozen until reload: {message}")));
                }
...
impl WorkerService for LiveWorker {
    fn build_id(&self) -> ProtocolDigest { self.build }
    fn active_scene(&self) -> Option<&str> { Some(&self.name) }
    fn last_state_hash(&self) -> Option<ProtocolDigest> { self.state_hash }
    fn begin_session(&mut self, supervisor: ProtocolDigest, max_bytes: usize) -> Result<(), ServiceError> {
        Python::attach(|py| with_capture(self.scene.bind(py), |c| c.recorded.begin_session(supervisor, max_bytes)))
    }
...
pub(crate) fn serve(scene: &Bound<'_, PyScene>) -> PyResult<()> {
    ...
    let result = scene.py().detach(move || {
        fmn_studio::serve_worker(&mut worker, &mut std::io::stdin().lock(),
            &mut std::io::stdout().lock(), ProtocolLimits::default())
...
fn button(value: MouseButton) -> u32 {
    match value { MouseButton::Left => 1, MouseButton::Middle => 2, MouseButton::Right => 4, MouseButton::Other(v) => u32::from(v) }
}
fn key(value: Key) -> u32 {
    match value {
        Key::Character(c) => c as u32, Key::Backspace => 0xff08, Key::Tab => 0xff09,
        Key::Enter => 0xff0d, Key::Escape => 0xff1b, Key::ArrowLeft => 0xff51,
        Key::ArrowUp => 0xff52, Key::ArrowRight => 0xff53, Key::ArrowDown => 0xff54,
        Key::Other(v) => v,
    }
}
...
fn dispatch(scene: &Bound<'_, PyScene>, event: EventPayload) -> PyResult<()> {
    ...
    match event {
        EventPayload::MouseMotion { point: p, delta, .. } => { scene.call_method1("on_mouse_motion", (point(p, false)?, point(delta, true)?))?; }
        EventPayload::MousePress { point: p, button: b, modifiers: m } => { scene.call_method1("on_mouse_press", (point(p, false)?, button(b), modifiers(m)))?; }
        EventPayload::MouseRelease { point: p, button: b, modifiers: m } => { scene.call_method1("on_mouse_release", (point(p, false)?, button(b), modifiers(m)))?; }
        EventPayload::MouseDrag { point: p, delta, button: b, modifiers: m } => { scene.call_method1("on_mouse_drag", (point(p, false)?, point(delta, true)?, button(b), modifiers(m)))?; }
        EventPayload::MouseScroll { point: p, offset, .. } => {
            ...
            let height = scene.getattr("camera")?.call_method0("get_pixel_height")?.extract::<u32>()?;
            if height == 0 { return Err(PyValueError::new_err("live camera has zero pixel height")); }
            let pixel_size = camera.height() / f64::from(height);
            let vector = np.call_method1("array", ([offset[0] * pixel_size, -offset[1] * pixel_size, 0.0],))?;
            scene.call_method1("on_mouse_scroll", (point(p, false)?, vector, offset[0], -offset[1]))?;
        }
        EventPayload::KeyPress { key: k, modifiers: m } => { scene.call_method1("on_key_press", (key(k), modifiers(m)))?; }
        EventPayload::KeyRelease { key: k, modifiers: m } => { scene.call_method1("on_key_release", (key(k), modifiers(m)))?; }
    }
    Ok(())
}
...
"#).unwrap();
            ...
            worker.handle(SupervisorRequest::Scrub { scene: "Live".into(), frame: 1 }).unwrap();
            let command = |revision, k| studio_input_command("Live", &StudioInput {
            ...
            assert!(worker.handle(SupervisorRequest::Play { scene: "Live".into(), command: command(1, 'a') }).is_err());
            let WorkerResponse::StudioData { bytes, .. } = worker.handle(SupervisorRequest::Inspect { scene: "Live".into() }).unwrap() else { panic!() };
            assert!(String::from_utf8(bytes).unwrap().contains("\"input_events\":false"));
