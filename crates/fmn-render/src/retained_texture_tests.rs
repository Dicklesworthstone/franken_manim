...
use fmn_mobject::{ImageColorSpace, ImageResource, ImageSampler, Mobject, RecordBuffer, RecordSchema};

const POINTS: [[f32; 3]; 4] = [
    [-3.0, -2.0, 0.0], [-3.0, 2.0, 0.0], [3.0, -2.0, 0.0], [3.0, 2.0, 0.0],
];
const UV: [[f32; 2]; 4] = [[0.0, 1.0], [0.0, 0.0], [1.0, 1.0], [1.0, 0.0]];
const PIXELS: [u8; 16] = [255, 0, 0, 255, 0, 255, 0, 128, 0, 0, 255, 0, 255, 255, 255, 255];

fn config(threads: usize) -> RetainedFrameRendererConfig {
    RetainedFrameRendererConfig {
        frame: FrameConfig::new(Viewport { width: 32, height: 18 },
            ScreenMap { scale: 2.25, origin: [16.0, 9.0] },
...
fn camera() -> Camera {
    Camera::new(CameraConfig { resolution: (32, 18), background: config(1).frame.background,
        ..CameraConfig::default() }).unwrap()
}

fn resource(pixels: Vec<u8>) -> ImageResource {
    ImageResource::rgba8(2, 2, pixels, ImageColorSpace::Linear, ImageSampler::default()).unwrap()
}

fn surface() -> Mobject {
    let schema = RecordSchema::new(
        &[("point", 3), ("d_normal_point", 3), ("im_coords", 2), ("opacity", 1)],
...
fn retained_textured_grid_matches_the_existing_texture_shader_at_all_thread_counts() {
    ...
    let vertices = (0..4).map(|i| SurfaceVertex::textured(
        POINTS[i].map(f64::from), [0.0, 0.0, 1.0], UV[i].map(f64::from), 0.75,
    )).collect();
    ...
    ThreeDJob::new(&camera, &draws, config(1).tiling).unwrap()
        .render_into(1, &mut reference.frame).unwrap();
    let mut background = RetainedFrameRenderer::new(config(1)).unwrap();
    background.render_with_camera(&Stage::new(), &camera).unwrap();
    ...
    for threads in [1, 4, 16] {
        ...
        assert_eq!(renderer.frame().as_bytes(), reference.frame().as_bytes(), "{threads}");
        assert_eq!(renderer.plan().images().len(), 1);
        assert_eq!(stage.get(mob).unwrap().render_primitive(),
...
fn retained_textured_surface_survives_copy_and_durable_snapshot_and_observes_live_changes() {
    ...
    assert_eq!(renderer.plan().images().len(), 1);
    stage.set_image_resource(second, Some(resource([255, 255, 0, 255].repeat(4)))).unwrap();
    renderer.render_with_camera(&stage, &camera).unwrap();
    ...
    assert_eq!(renderer.plan().images().len(), 1);
    stage.get_mut(second).unwrap().buffer.write_range("im_coords", 0, &[0.25; 8]);
    renderer.render_with_camera(&stage, &camera).unwrap();
    assert_ne!(renderer.frame().as_bytes(), initial);
...
fn invalid_texture_records_leave_the_last_good_frame_and_resource_table_unchanged() {
    ...
    stage.get_mut(mob).unwrap().buffer.write(0, "im_coords", &[f32::NAN, 0.0]);
