...
pub(super) fn selected(command: &RenderCommand) -> bool {
    command.file.as_deref() == Some(Path::new(BUILTIN_SCENE_SOURCE))
        && command.scene_names.iter().any(|name| name == fmn::builtins::INTERACTIVE_SCENE_NAME)
}

pub(super) fn validate_selection(command: &RenderCommand) -> Result<(), CliError> {
    if command.scene_names.len() != 1 || command.write_all {
        return Err(CliError::new("scene", "live Studio requires exactly one registered scene"));
    }
    if command.skip_animations || command.animation_range.is_some() || command.presenter_mode {
        return Err(CliError::new("config", "live Studio uses its timeline controls, not offline skip/range/presenter flags"));
    }
    Ok(())
}

fn configuration(fs: &dyn FileSystem, command: &StudioCommand) -> Result<(NativeWorkerConfig, fmn_scene::RuntimeConfig, u64), CliError> {
    ...
    let engine = match plan.engine {
        ...
        _ => return Err(CliError::new("capability", "live native Studio currently requires a CPU renderer; no annex fallback is substituted")),
    };
    let renderer = RetainedFrameRendererConfig {
        ...
        engine, threads: plan.render_teams.first().map_or(1, fmn_runtime::TeamPlan::threads),
    };
...
pub(super) fn source_reads(fs: &dyn FileSystem, command: &StudioCommand) -> Result<Vec<AssetRead>, CliError> {
    if selected(&command.render) { Ok(configuration(fs, command)?.0.base_input_reads()) }
    else { Ok(Vec::new()) }
}

pub(super) fn worker(fs: &dyn FileSystem, command: &StudioCommand) -> Result<Box<dyn fmn_studio::WorkerService>, CliError> {
    if !selected(&command.render) { return Ok(Box::new(NativeStudioWorker::from_command(fs, command)?)); }
    ...
    let worker = NativeSceneWorker::new(config, move || {
        let error = |error: String| fmn_studio::ServiceError::new(fmn_studio::WorkerErrorCode::ExecutionFailed, error);
        let mut scene = fmn_scene::Scene::new(runtime.clone(), seed).map_err(|e| error(e.to_string()))?;
        fmn::builtins::populate_interactive_canvas(scene.stage_mut()).map_err(|e| error(e.to_string()))?;
        NativeSceneProgram::new(scene, vec![NativeSegment::Wait { duration: Some(2.0) }], limit)
...
mod tests {
    ...
    fn command() -> StudioCommand {
        let Invocation::Studio(command) = parse_args(["studio", "--resolution", "96x54", "--fps", "8", "--threads", "1", "@builtin", "interactive.v1"]).unwrap() else { panic!("Studio"); };
        command
    }
    #[test]
    fn live_registration_exposes_constructed_zero_and_native_windowed_clock() {
        ...
        let fmn_studio::WorkerResponse::StudioData { bytes, .. } = service.handle(fmn_studio::SupervisorRequest::Inspect { scene: scene.clone() }).unwrap() else { panic!("inspector"); };
        ...
        assert!(matches!(service.handle(fmn_studio::SupervisorRequest::Scrub { scene: scene.clone(), frame: 60 }).unwrap(), fmn_studio::WorkerResponse::Frame(_)));
        assert!(service.handle(fmn_studio::SupervisorRequest::Scrub { scene, frame: 61 }).is_err());
        assert_eq!(source_reads(&fs, &command).unwrap().len(), 4);
    }
    #[test]
    fn live_factory_rejects_offline_skip_before_execution() {
        let mut command = command(); command.render.skip_animations = true;
