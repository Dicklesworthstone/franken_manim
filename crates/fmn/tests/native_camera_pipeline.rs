...
use fmn_render::{Camera, EngineIdentity, FrameConfig, RetainedFrameRenderer, RetainedFrameRendererConfig, ScreenMap, Tiling, Viewport};
...
fn config(plan: &ExecutionPlan, camera: &CameraConfig) -> RetainedFrameRendererConfig {
    RetainedFrameRendererConfig {
        frame: FrameConfig::new(Viewport { width: 48, height: 32 },
            ScreenMap { scale: 8.0, origin: [24.0, 16.0], y_up: true }, camera.background),
...
fn populate(scene: &mut Scene) -> fmn::Result<()> {
    ...
    scene.stage_mut().set_fill(circle, Some(WHITE), Some(1.0), None, true);
...
fn target(base: &CameraConfig) -> CameraConfig {
    ...
    target.frame.set_euler_angles(Some(0.6), Some(0.8), None).unwrap();
...
struct Orbit { rig: CameraRig, target: CameraConfig }
impl SceneConstruct for Orbit {
    fn construct(&mut self, stage: &mut Stage<'_>) -> fmn::Result<()> {
        ...
        stage.play_prepared_with(vec![Box::new(animation)], PlayOverrides {
            run_time: Some(0.5), rate_func: Some(fmn::core::rate::linear),
...
fn factory(scene: &mut Scene, base: &CameraConfig) -> fmn::Result<(Orbit, CameraRig)> {
    ...
    Ok((Orbit { rig, target: target(base) }, rig))
}

struct Serial {
    rig: CameraRig, base: CameraConfig, config: RetainedFrameRendererConfig,
    images: Vec<Vec<u8>>,
}
...
fn images(fs: &VirtualFs, path: &str, count: u64) -> Vec<Vec<u8>> {
    (0..count).map(|index| {
        decode_png(&fs.read(&Path::new(path).join(format!("frame_{index:06}.png"))).unwrap(),
            &PngLimits::default()).unwrap().rgba
    }).collect()
}
...
fn camera_orbit_zoom_fov_and_light_match_serial_snapshots_at_every_window() {
    ...
    for window in [1, 2, 4] {
        ...
        let mut scene = Scene::new(RuntimeConfig::from_config(&options.config), options.config.determinism.seed).unwrap();
        ...
        assert_eq!((stats.submitted, stats.emitted, stats.outstanding_slots), (4, 4, 0));
        ...
        let mut serial = Serial { rig, base: base.clone(), config: config(&report.execution_plan, &base), images: Vec::new() };
        scene.play(vec![Box::new(animation)], PlayOverrides {
            run_time: Some(0.5), rate_func: Some(fmn::core::rate::linear), ..PlayOverrides::default()
        }, &mut serial).unwrap();
        let got = images(&fs, "/orbit", 4);
        assert_eq!(got, serial.images);
        assert_ne!(got[0], got[3], "the camera, not the geometry, must move");
        if let Some(first) = &first { assert_eq!(&got, first); } else { first = Some(got); }
    }
}
...
fn non_send_camera_updaters_execute_on_scene_owner_and_affect_the_same_capture() {
    ...
    let base = options.camera_config().unwrap();
    let report = render_camera_with_fs(move |scene, base| {
        let rig = CameraRig::new(scene, base)?;
        scene.stage_mut().add_dt_updater(rig.center()[0], move |stage, mob, dt| {
    ...
    for index in 1..=4 {
        ...
        pose.frame.set_center([f64::from(index) / 8.0, 0.0, 0.0]).unwrap();
        let mut renderer = RetainedFrameRenderer::new(config(&report.execution_plan, &base)).unwrap();
        renderer.render_with_camera(scene.stage(), &Camera::new(pose).unwrap()).unwrap();
        let mut pixels = FrameBuffer::new(FrameLayout::tight(PixelFormat::Rgba8, 48, 32).unwrap());
        rgba16f_to_rgba8(renderer.frame(), &mut pixels).unwrap();
...
fn invalid_camera_changes_do_not_consume_frames_or_replace_a_good_pose() {
    ...
    let plan = ExecutionPlan::derive(
        PlanRequest::certified(RenderIntent::Offline, SurfaceSpec::lumen(48, 32), OutputPixelFormat::Rgba8)
            .with_max_frames_in_flight(4),
        &HardwareTopology::from_group_sizes(&[2, 2]).unwrap(), None,
    ).unwrap();
    let received = Arc::new(Mutex::new(Vec::new()));
    let copy = received.clone();
    let emitter = OrderedEmitter::new(EmitterConfig::new(
        FrameLayout::tight(PixelFormat::Rgba8, 48, 32).unwrap(), plan.frames_in_flight, 0,
    ).unwrap(), vec![SinkBinding::reliable("record", move |_, frame: &FrameBuffer| {
        copy.lock().unwrap().push(frame.as_bytes().to_vec()); Ok(SinkWrite::Consumed)
    })]).unwrap();
    let mut pipeline = NativeFramePipeline::new(plan.clone(), config(&plan, &base), Some(Camera::new(base.clone()).unwrap()), emitter.handle()).unwrap();
    let mut scene = Scene::default(); populate(&mut scene).unwrap();
...
fn invalid_rigs_and_factory_playback_abort_without_publication() {
    ...
    let fs = Arc::new(VirtualFs::new());
    for kind in 0..3 {
        let result = render_camera_with_fs(|scene, base| {
            let rig = if kind == 0 {
                CameraRig::new(&mut Scene::default(), base)?
            } else { CameraRig::new(scene, base)? };
            if kind == 1 { scene.stage_mut().detach(rig.root(), rig.width()); }
            if kind == 2 { scene.wait(Some(0.125), &mut NullSceneSink)?; }
            Ok((Empty, rig))
        }, options("/retry", 4), fs.clone());
        assert!(result.is_err()); assert!(!fs.exists(Path::new("/retry")));
    }
    let report = render_camera_with_fs(|scene, base| Ok((Empty, CameraRig::new(scene, base)?)),
        options("/retry", 4), fs.clone()).unwrap();
...
fn bad_options_do_not_run_factory_and_scene_unwind_joins_workers() {
    let fs = Arc::new(VirtualFs::new());
    let mut invalid = options("/bad", 4); invalid.frames_in_flight = 0;
    let called = Cell::new(false);
    assert!(render_camera_with_fs(|scene, base| {
        called.set(true);
        factory(scene, base)
    }, invalid, fs.clone()).is_err());
    assert!(!called.get());
    struct Panics;
    impl SceneConstruct for Panics {
        fn construct(&mut self, stage: &mut Stage<'_>) -> fmn::Result<()> {
            populate(stage.scene_mut())?; stage.wait(0.25)?;
            panic!("abort after camera captures")
        }
    }
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        render_camera_with_fs(|scene, base| Ok((Panics, CameraRig::new(scene, base)?)),
            options("/retry", 4), fs.clone())
    }));
    assert!(result.is_err()); assert!(!fs.exists(Path::new("/retry")));
...
fn camera_animation_gif_and_y4m_are_window_independent() {
    for format in [RenderFormat::Gif, RenderFormat::Y4m] {
        ...
        for window in [1, 2, 4] {
            let fs = Arc::new(VirtualFs::new());
            let mut options = options("/movie", window); options.format = format;
            let report = render_camera_with_fs(factory, options, fs.clone()).unwrap();
