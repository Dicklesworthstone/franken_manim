...
impl SceneConstruct for StatefulScene {
    fn construct(&mut self, stage: &mut Stage<'_>) -> fmn::Result<()> {
        ...
        stage.add_updater(follower, move |stage, me| {
...
fn exported_stateful_scene_replays_identical_png_bytes_at_different_thread_counts() {
    ...
    let receipt = export_bundle_with_fs(
        &mut program as &mut dyn SceneConstruct, "/scene.fmtl", options(), fs.clone(),
    ).unwrap();
    ...
    let direct = render_with_fs(
        ...
        render_options("/direct", &config, 1), fs.clone(),
    ).unwrap();
    ...
    for threads in [1, 4] {
        ...
        let replay = render_bundle_with_fs(
            &bytes, render_options(&output, &config, threads), fs.clone(),
        ).unwrap();
    ...
    assert_eq!(recorded_calls.get(), calls_at_export, "replay cannot invoke source callbacks");
    let again = export_bundle_bytes(
        &mut StatefulScene(Rc::new(Cell::new(0))), options(),
    ).unwrap();
...
fn static_scene_has_one_replayable_still_without_advancing_scene_time() {
    ...
    assert!(!TimelineBundle::from_bytes(&artifact.bundle.bytes).unwrap().stage_at(0).unwrap().roots().is_empty());
}
...
fn scene_errors_and_output_budgets_never_publish_partial_files() {
    let fs = Arc::new(VirtualFs::new());
    let error = export_bundle_with_fs(&mut FailingScene, "/failed.fmtl", options(), fs.clone()).unwrap_err();
    assert!(matches!(error, BundleExportError::Scene(fmn::Error::Scene(
        SceneError::InvalidLifecycle("deliberate post-capture failure")
    ))));
    assert!(!fs.exists(Path::new("/failed.fmtl")));
    let mut limited = options();
    limited.max_output_bytes = 1;
    assert!(matches!(export_bundle_with_fs(&mut StaticScene, "/large.fmtl", limited, fs.clone()),
...
fn publication_never_overwrites_an_existing_destination() {
    let fs = Arc::new(VirtualFs::new());
    fs.write_atomic(Path::new("/occupied.fmtl"), b"existing project data").unwrap();
    assert!(matches!(export_bundle_with_fs(&mut StaticScene, "/occupied.fmtl", options(), fs.clone()),
        Err(BundleExportError::FileSystem(_))));
    assert_eq!(fs.read(Path::new("/occupied.fmtl")).unwrap(), b"existing project data");
}
...
fn swallowed_sink_failure_still_refuses_the_whole_export() {
    ...
    assert!(matches!(export_bundle_with_fs(&mut SwallowedCaptureFailure, "/partial.fmtl", options, fs.clone()),
        Err(BundleExportError::Recording(_))));
...
fn unrepresentable_audio_is_an_explicit_capability_error() {
    let fs = Arc::new(VirtualFs::new());
    assert!(matches!(export_bundle_with_fs(&mut AudioScene, "/audio.fmtl", options(), fs.clone()),
        Err(BundleExportError::Capability(_))));
...
fn invalid_options_are_preflighted_and_unwinding_publishes_nothing() {
    for field in 0..4 {
        ...
        assert!(matches!(export_bundle_bytes(&mut MustNotRun, options),
            Err(BundleExportError::InvalidOptions(_))));
    }
    let fs = Arc::new(VirtualFs::new());
    assert!(matches!(export_bundle_with_fs(&mut MustNotRun, "", options(), fs.clone()),
        Err(BundleExportError::InvalidOptions(_))));
...
fn normal_early_scene_termination_keeps_its_completed_frames() {
    ...
    assert_eq!(i64::from(export.bundle.frame_count), export.scene.time.frames());
