...
use fmn::rendering::{NativeFramePipeline, RenderError, RenderFormat, RenderOptions, render_bundle_with_fs};
...
use fmn_scene::timeline_bundle::{BundleSegmentKind, SharedTimelineBundle, TimelineBundle, export_timeline_bundle};
...
fn bundle(camera: bool, stateful: bool) -> Vec<u8> {
    ...
    let mob = stage.add(if camera { fmn::library::Cube::new(1.0).into() } else { vector() });
    stage.add_to_scene(mob).unwrap();
    let mut timeline = Timeline::new(8).unwrap();
    if stateful {
        stage.add_dt_updater(mob, |stage, mob, dt| { stage.shift(mob, [dt, 0.0, 0.0]); }, false).unwrap();
        timeline.wait(0.5).unwrap();
    } else {
        let builder = mob.animate().set_anim_args(AnimateArgs {
            run_time: Some(0.5), rate_func: Some(rate::linear), ..AnimateArgs::default()
        }).unwrap().shift([1.0, 0.0, 0.0]).unwrap();
...
fn options(path: &str, window: usize, camera: bool) -> RenderOptions {
    ...
    if camera {
        ...
        config.frame.set_euler_angles(Some(0.4), Some(0.7), None).unwrap();
...
fn config(plan: &ExecutionPlan, options: &RenderOptions) -> RetainedFrameRendererConfig {
    RetainedFrameRendererConfig {
        frame: FrameConfig::new(Viewport { width: 32, height: 24 },
            ScreenMap { scale: 4.0, origin: [16.0, 12.0], y_up: true },
            fmn::core::color::Srgb::from_hex(&options.config.camera.background_color).unwrap()
...
fn compiled_pure_and_stateful_captures_match_serial_pixels_on_every_render_team() {
    for camera in [false, true] {
        for stateful in [false, true] {
            ...
            assert_eq!(serial.segment_kind(0), Some(if stateful { BundleSegmentKind::Stateful } else { BundleSegmentKind::Pure }));
            let shared = SharedTimelineBundle::from_bytes(&bytes).unwrap();
            let plan = ExecutionPlan::derive(
                PlanRequest::certified(RenderIntent::Offline, SurfaceSpec::lumen(32, 24), OutputPixelFormat::Rgba8)
            ...
            let emitter = OrderedEmitter::new(EmitterConfig::new(
                FrameLayout::tight(PixelFormat::Rgba8, 32, 24).unwrap(), plan.frames_in_flight, 0,
            ).unwrap(), vec![SinkBinding::reliable("record", move |sequence, frame: &FrameBuffer| {
                output.lock().unwrap().push((sequence, frame.as_bytes().to_vec())); Ok(SinkWrite::Consumed)
            })]).unwrap();
            let mut pipeline = NativeFramePipeline::new(plan.clone(), config, camera.clone(), emitter.handle()).unwrap();
            let jobs: Vec<_> = [3, 0, 2, 1].into_iter().map(|index| shared.frame_job(index).unwrap()).collect();
            drop(shared); drop(bytes);
            ...
            for (sequence, job) in jobs.into_iter().enumerate() {
                ...
                if let Some(camera) = &camera { renderer.render_with_camera(&stage, camera).unwrap(); }
                else { renderer.render(&stage, 0).unwrap(); }
    ...
    for camera in [false, true] {
        for format in [RenderFormat::PngSequence, RenderFormat::Gif, RenderFormat::Y4m] {
            ...
            for window in [1, 2, 4] {
                ...
                let mut options = options("/output", window, camera); options.format = format;
                ...
                let frames = if format == RenderFormat::PngSequence {
                    (0..4).map(|index| fs.read(&Path::new("/output").join(format!("frame_{index:06}.png"))).unwrap()).collect::<Vec<_>>()
                } else { vec![fs.read(Path::new("/output")).unwrap()] };
                if let Some(first) = &first { assert_eq!(&frames, first); } else { first = Some(frames); }
            }
        }
    }
}
...
fn compiled_refusals_do_not_publish_and_worker_failure_allows_retry() {
    ...
    let mut wrong_fps = options("/retry", 4, false); wrong_fps.config.camera.fps = 10;
    assert!(matches!(render_bundle_with_fs(&bytes, wrong_fps, fs.clone()), Err(RenderError::InvalidOptions(_))));
    let mut limited = options("/retry", 4, false); limited.max_frames = 3;
    assert!(matches!(render_bundle_with_fs(&bytes, limited, fs.clone()), Err(RenderError::InvalidOptions(_))));
    assert!(!fs.exists(Path::new("/retry")));
    // A 3D bundle on the affine route fails inside a real render worker.
    assert!(matches!(render_bundle_with_fs(&bundle(true, false), options("/retry", 4, false), fs.clone()), Err(RenderError::Pipeline(_))));
    assert!(!fs.exists(Path::new("/retry")));
...
fn empty_bundle_and_existing_native_outputs_are_not_silently_replaced() {
    let mut stage = Stage::new();
    let empty = export_timeline_bundle(Timeline::new(8).unwrap(), &mut stage, &RngRoot::from_seed(0)).unwrap();
    let fs = Arc::new(VirtualFs::new());
    ...
    for format in [RenderFormat::Gif, RenderFormat::Y4m] {
        ...
        let mut options = options("/existing", 2, false); options.format = format;
        assert!(render_bundle_with_fs(&bundle(false, false), options, fs.clone()).is_err());
        assert_eq!(fs.read(Path::new("/existing")).unwrap(), b"owned by someone else");
    }
}
