...
impl SceneConstruct for ApexUp {
    fn construct(&mut self, stage: &mut Stage<'_>) -> fmn::Result<()> {
        let triangle = stage.add(Polygon::new([
...
fn png(fs: &VirtualFs, path: &str) -> fmn_codec::DecodedPng {
    decode_png(&fs.read(Path::new(path)).expect("published PNG"), &PngLimits::default())
...
fn lit_pixels_on_row(image: &fmn_codec::DecodedPng, row: usize) -> usize {
    ...
    image.rgba[row * width * 4..(row + 1) * width * 4]
        .chunks_exact(4).filter(|pixel| pixel[0] > 128).count()
}

#[test]
fn static_scene_produces_one_nonuniform_upright_png() {
    let fs = Arc::new(VirtualFs::new());
    let report = render_with_fs(&mut ApexUp, options("/static", RenderFormat::PngSequence, 1), fs.clone())
    ...
    assert!(lower > upper * 2, "object +Y is image-up, not mirrored: {upper}/{lower}");
    assert!(lower < image.width as usize, "the background is distinct from the triangle");
}
...
fn certified_png_bytes_are_invariant_under_thread_caps() {
    ...
    for threads in [1, 4, 16] {
        let fs = Arc::new(VirtualFs::new());
        let report = render_with_fs(&mut MovingCircle, options("/moving", RenderFormat::PngSequence, threads), fs.clone())
            .expect("certified export");
        let frames = (0..report.artifact.frame_count)
            .map(|index| fs.read(&Path::new("/moving").join(format!("frame_{index:06}.png"))).unwrap())
            .collect::<Vec<_>>();
...
fn native_gif_and_y4m_are_complete_real_streams() {
    for (path, format) in [("/clip.gif", RenderFormat::Gif), ("/clip.y4m", RenderFormat::Y4m)] {
        let fs = Arc::new(VirtualFs::new());
...
fn scene_error_preserves_source_and_aborts_all_formats_before_return() {
    for format in [RenderFormat::PngSequence, RenderFormat::Gif, RenderFormat::Y4m] {
        ...
        assert!(matches!(error, RenderError::Scene(fmn::Error::Scene(
            SceneError::InvalidConfig("scene's original failure")
        ))));
...
fn frame_budget_failure_does_not_publish_a_truncated_animation() {
    ...
    config.max_frames = 1;
    assert!(matches!(render_with_fs(&mut MovingCircle, config, fs.clone()),
...
fn existing_artifacts_are_not_clobbered() {
    ...
    assert!(render_with_fs(&mut MovingCircle, options("/keep.y4m", RenderFormat::Y4m, 1), fs.clone()).is_err());
    assert_eq!(fs.read(Path::new("/keep.y4m")).unwrap(), b"user-owned artifact");
}
...
fn invalid_options_fail_without_publishing_or_running_the_scene() {
    ...
    for case in 0..8 {
        ...
        assert!(render_with_fs(&mut MustNotRun, config, fs.clone()).is_err(), "case {case}");
...
fn unwinding_scene_panic_aborts_and_joins_output() {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let _ = render_with_fs(&mut Panicking, options("/panic", RenderFormat::PngSequence, 1), fs.clone());
    }));
