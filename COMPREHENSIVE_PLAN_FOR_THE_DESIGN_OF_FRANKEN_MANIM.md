...
**16.8 The Behavior Notes.** The user-facing register of deliberate differences from classic manim — one evidence-backed entry each, written as migration guidance. **The register itself is `docs/behavior_notes/README.md`**, which is authoritative and carries the numbering rules (one number per note, never reused; a number may name a family where this section does; reserved numbers stay reserved; `Draft` → `Final` on gate passage) — so a workstream that finds a deliberate divergence writes its note and takes the next free number without amending this plan (ADR-0009). The seeds this section named, which remain the register's first rows: BN-01 single RNG (PCG64DXSM; seeded scenes reproduce within FrankenManim, not across engines); BN-02 rational clock on manim's sample points (no drift); BN-03 true arc length under the original names (constant-speed paths; length-true dashes/tips); BN-04 color: linear-light compositing with manim's gradient formulas; BN-05 native typesetting (metrics differ from LaTeX; quality bar documented); BN-06 renderer (analytic coverage, round caps, arclength stroke width — strokes and fills are cleaner than the GPU pipeline's), reserved for W5; BN-07 Reference bugs fixed (Appendix C rulings — a family, since Appendix C's rulings are owned by different workstreams); BN-08 de-TeX'd classes (§11.6); BN-09 one arc-density rule; BN-10 skip mode delivers playback's updater time (§9.3); BN-11 composition honors its declared timing (C-10, C-11); BN-12 the Animation contract's typed edges (§9.1).
...
D-01 The contract is API/semantic compatibility + correctness + beauty; output identity with Python manim is a non-goal. D-02 **ffmpeg is the sole external tool**, optional, sandboxed, excluded from certified artifacts. D-03 **No LaTeX on any path**: mathematics is fmd-math, living as new workspace crates (`fmd-font`, `fmd-math`) in franken_markdown, governed like foundation crates. D-04 Lumen is one analytic renderer *semantically*, executed by multiple engines that share the IR and the tests (§10.1); the Reference's lighting, projection, palette, and AA feel are kept as the calibrated look. D-05 manim names carry correct semantics (arclength, clock, RNG, color), documented in the Behavior Notes. D-06 One RNG: PCG64DXSM with named substreams. D-07 The RationalFrameClock emits manim's nominal sample points exactly, in rational time. D-08 Default face: bundled Computer Modern; nothing depends on host fonts. D-09 Spans come from native layout provenance; the two-render alignment hack is dead; assignment matching remains for shape-based transforms only. D-10 De-TeX the incidental classes (§11.6). D-11 Arena + handles **ratified with amendments by G0-1** (2026-07-22, fm-dzv): stage-scoped generational handles, root-set scene membership, delete-defers-under-pins proxy identity, the snapshot/view exclusion rule for §8.2 (R12's copy-based fallback not needed), and the scoped-stage + deferred-command fluent surface — see `docs/g0/G0-1-object-model-ratification.md` and `spikes/g0-1-object-model`. D-12 Governed-closure doctrine (§3). D-13 Custom GLSL excluded from the compatibility claim; a short corpus-adapter list only. D-14 Supervisor + worker iteration; multipart-PNG Studio baseline. D-15 Exact pinned nightly; scalar-first as ordering; **SIMD is `std::simd` with safe `#[target_feature]` in build tiers** governed by SUITE.lock's certified target-feature sets, under the §10.5 lane rules — the abstraction is now named, and no per-call `unsafe` dispatch exists. D-16 Self-goldens are the regression gate; Reference imagery is reference. D-17 fmn-dmath owns certified transcendentals; fmn-hash owns content addressing. D-18 **The parallelism contract** (§10.5 a–f) binds every optimization; its three refusals — GPU in certified, variable frame sampling, completion-order RNG — are permanent. D-19 Frames freeze into immutable **FramePackets** after capture (§9.3); **pure segments** render frame-parallel (§9.5); execution pipelines behind a bounded, RAM-derived in-flight budget with ordered emit (§17.4). D-20 The render plan is **retained**: mandatory lazy revisioned mirrors, compiled-path caches, glyph/shape instancing, two-level binning, painter-safe occlusion pruning, and the retained compositor — in export and preview alike (§8.2, §10.8). D-21 **Adaptive coverage** replaces full-frame SSAA as the standard default; `certified` runs the canonical path everywhere (§10.4). D-22 The **Accelerator Annex** reaches GPUs only through frankentorch (Metal now; CUDA on the upstream ledger); no wgpu; never certified; PG-A-gated only; Studio preview first (§10.7). D-23 The output sink is **negotiated** (§14.3): native orientation and transfer, NV12/P010, hardware encoders as a standard-mode knob, asynchronous rings; `vflip`/`eq` are dead. D-24 Scheduling derives from introspected **HardwareTopology** via an **ExecutionPlan**; autotune caches are standard-only; certified runs its declared fixed configuration (§17.4).

**Open questions (owned; resolved at G0 or the named workstream).**
OQ-1 (G0-6) Floating + fmn-dmath vs fixed-point raster boundary for certification — **resolved 2026-07-25, ADR-0010** (fm-zn9): **floating suffices; the fixed-point raster boundary is retired, not deferred.** One frame exercising gradient fills, winding holes, joins, per-vertex atan2, the true_dot glow, rate functions and alpha compositing hashed **byte-identically on linux-x86_64 (glibc, native), linux-aarch64 (musl, qemu) and macos-aarch64 (Darwin, M4 Pro)** — all 18 digests, including every fmn-dmath function. No precision or rounding constants are needed. The result rests on four properties now binding on W5: fmn-dmath owns every certified transcendental; no FMA contraction (verified in aarch64 object code — 1652 scalar FP ops, zero fmadd/fmla); fixed-order reductions; IEEE-754 basic operations only. Evidence: `docs/g0/G0-6-determinism-ratification.md`, raw data `docs/g0/g0-6-hashes/`. OQ-2 (G0-3) Extensible delimiters: CM glyph assembly, drawn paths, or hybrid — and the metrics-synthesis calibration method — **resolved 2026-07-23, ADR-0005** (fm-nh0): drawn-path mainline (natural glyph → ≤1.25× uniform scale → parametric drawn construction; assembly rejected — CM Unicode ships no extension repertoire); metrics = published TFM fontdimens validated against fmd-font-decoded geometry (see `docs/g0/G0-3-fmd-math-ratification.md`). OQ-3 (W6) fmd workspace-conversion mechanics and release/versioning for the new crates — **resolved 2026-07-23, ADR-0004** (fm-ydw): root-package workspace with an `fmd-font` member re-exported as `text`; fmd-font 0.1.0 riding fmd's release cadence; consumed here as a git dependency at the SUITE.lock rev. OQ-4 (W6) Text-mode TeX (`TexText`) surface breadth for tier-1 — **resolved 2026-07-24, ADR-0006** (fm-7dw): mainland (escapes, ties, comments) + `$…$`/`$$…$$` math islands at full math tier-1 + `\textbf`/`\emph`/`\underline`, with the missing-`$` recovery kept; everything else refuses by name, scheduled by corpus rank. OQ-5 (W7) fnx layout-kernel audit outcome. OQ-6 (post-W1) Windows bit-certification. OQ-7 (W8) Container/codec surface beyond the Reference's. OQ-8 (banked) A restricted GLSL interpreter, only on demonstrated demand. OQ-9 (W10) Whether the gallery keeps any Strategy-B adapters or ships exclusions only. OQ-10 (G0-8) Whether CUDA-via-ft reaches annex quality now or waits on upstream ft device work — **resolved 2026-07-25, ADR-0007** (fm-ekx): it waits, on two named preconditions — the ft gateway's *buffer* API must generalize to device-resident handles with delta upload (§17.6's resident-scene playbook; unified memory is an Apple luxury, and the dispatch API and kernel port unchanged), and D1 must rule on shipping precompiled PTX before any CUDA code exists. The Metal half is proven end-to-end (see `docs/g0/G0-8-accelerator-ratification.md`); the reachable NVIDIA part is not PG-A's declared profile, so nothing measured there may be reported as a PG-A number. OQ-11 (W5) Render-team sizing priors per scene class — the autotuner's starting points (e.g. 3×32 vs 4×24 on 96 cores). OQ-12 (G3) Whether the annex-preview fallback is exercised (see G3's declared fallback and R21).
...
| surface / textured_surface | UV-grid tessellation; `finalize_color` `(reflectiveness, gloss, shadow)`, Surface `(0.3,0.2,0.4)`; light (−10,10,10); `dark_shift=0.2` on the *textured* variant only, as a light/dark texture crossfade | **kept**: the lighting model is the look; ported exactly, and measured against the captures to 0.4 % (§10.4, G0-2 L5) |
