...
- [x] Commit the reviewed integration batch as `8e1a8a4061f2c15db7c9e434500176b6af5af5ba`; the tracked tree was clean immediately afterward. Subsequent video edits are a separate source window.
  ...
  - [x] Collect `committed-source-8e1a8a40-clean.log`: RCH vmi1264463 exits 137/SIGKILL during the bare-test Python group, after checks, Clippy, Rustdoc and many native tests pass. The corpus PNG test completed, but no whole `cargo test` or whole gate exit 0 exists. Read-only worker inspection found no matching kernel/systemd-oomd event; the cause remains unproven. Post-kill memory availability and disk space do not explain the earlier event.
  - [ ] Finish `committed-source-1eba1769.log` on hz3: full `scripts/check.sh` from a self-contained clean clone of `1eba1769851fadd92c943388cb83fc531815e105`, with both real pinned private corpus directories and `FMN_REQUIRE_FFMPEG=1`. No old worktree metadata, dirty waiver or missing-corpus skip is used. Closure decisions await its terminal result.
...
- [ ] `fm-5wq.4.145`: connect remaining Python console/output options to Reel; exercise real decoded outputs and encoder/error paths.
  ...
  - [ ] Repair the concrete live-camera capture gap before broadening console flags.
    ...
    - [ ] Complete `camera-native-and-wheel-1035.log`: added decoded scene-bound light changes and Python `get_center` override reversal pass the native output test (15.00 seconds). Registered E2E and a fresh CPython 3.13 wheel with required real ffmpeg follow in the same RCH job. The wheel source manifest binds ten source/test files; no whole wheel or full source pass is claimed before terminal receipts.
